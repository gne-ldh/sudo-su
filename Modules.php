...
if ((!$_SESSION['UserMP'] || (optional_param('college', '', PARAM_SPCL) && optional_param('college', '', PARAM_SPCL) != $old_college) || (optional_param('syear', 0, PARAM_SPCL) && optional_param('syear', 0, PARAM_SPCL) != $old_syear)) && User('PROFILE') != 'parent')
...
if (User('PROFILE') == 'teacher') {
    echo "<li><FORM name=head_frm id=head_frm action=Side.php?modfunc=update&btnn=$btn&nsc=$ns&act=college method=POST><div class=\"form-group\"><INPUT type=hidden name=modcat value='' id=modcat_input>";
    $RET = DBGet(DBQuery('SELECT s.ID,s.TITLE FROM colleges s,staff st INNER JOIN staff_college_relationship ssr USING(staff_id) WHERE s.id=ssr.college_id AND ssr.syear=\'' . UserSyear() . '\' AND st.staff_id=\'' . $_SESSION[STAFF_ID] . '\' AND (ssr.END_DATE>=curdate() OR ssr.END_DATE=\'0000-00-00\' OR ssr.END_DATE IS NULL)'));
    echo "<SELECT class=\"select-search\" style=\"width: 200px;\" name=college onChange='this.form.submit();'>";
    ...
    $college_years_RET = DBGet(DBQuery("SELECT YEAR(sy.START_DATE)AS START_DATE,YEAR(sy.END_DATE)AS END_DATE FROM college_years sy,staff st INNER JOIN staff_college_relationship ssr USING(staff_id) WHERE ssr.SYEAR=sy.SYEAR AND sy.college_id=ssr.college_id AND sy.college_id=" . UserCollege() . " AND st.staff_id=$_SESSION[STAFF_ID]"));
    echo "<SELECT class=\"select\" name=syear onChange='this.form.submit();' style='width:80;'>";
    foreach ($college_years_RET as $college_years) {
        echo "<OPTION value=$college_years[START_DATE]" . ((UserSyear() == $college_years['START_DATE']) ? ' SELECTED' : '') . ">$college_years[START_DATE]" . ($college_years[END_DATE] != $college_years[START_DATE] ? "-" . $college_years['END_DATE'] : '') . '</OPTION>';
    }
    echo '</SELECT>';
    echo "</div></FORM></li>";
...
if (User('PROFILE') != 'teacher') {
    ...
    if (User('PROFILE') == 'admin') {
        $RET = DBGet(DBQuery("SELECT DISTINCT s.ID,s.TITLE FROM colleges s,staff st INNER JOIN staff_college_relationship ssr USING(staff_id) WHERE s.id=ssr.college_id AND st.staff_id=$_SESSION[STAFF_ID] ORDER BY s.TITLE asc"));
        echo "<SELECT class=\"select-search\" name=college onChange='this.form.submit();'>";
        foreach ($RET as $college)
            echo "<OPTION  style='padding-right:8px;' value=$college[ID]" . ((UserCollege() == $college['ID']) ? ' SELECTED' : '') . ">" . $college['TITLE'] . "</OPTION>";
        echo "</SELECT>";
    }
    ...
    $college_years_RET1['END_DATE'] = explode("-", $college_years_RET1['END_DATE']);
    $college_years_RET1['END_DATE'] = $college_years_RET1['END_DATE'][0];
    ...
    if ($college_years_RET1['END_DATE'] > $college_years_RET1['START_DATE']) {
        ...
        foreach ($college_years_RET as $college_years) {
            ...
            $college_years['START_DATE'] = $college_years['START_DATE'][0];
            $college_years['END_DATE'] = explode("-", $college_years['END_DATE']);
            $college_years['END_DATE'] = $college_years['END_DATE'][0];
            echo "<OPTION value=$college_years[START_DATE]" . ((UserSyear() == $college_years['START_DATE']) ? ' SELECTED' : '') . "> $college_years[START_DATE]-" . ($college_years['END_DATE']) . "</OPTION>";
        }
        ...
        foreach ($college_years_RET as $college_years) {
            $college_years['START_DATE'] = explode("-", $college_years['START_DATE']);
            $college_years_RET['START_DATE'] = $college_years['START_DATE'][0];
            echo "<OPTION value=$college_years_RET[START_DATE]" . ((UserSyear() == $college_years_RET['START_DATE']) ? ' SELECTED' : '') . ">$college_years_RET[START_DATE]</OPTION>";
        }
    }
    #}
    echo '</SELECT>';
...
//===================================================================================================							     					     
    ...
    if (count($RET) > 0) {
        ...
        foreach ($RET as $period) {
            $x_sel = "";
            $period_det = DBGet(DBQuery('SELECT sp.TITLE as PERIOD_NAME,cpv.DAYS,cpv.COURSE_PERIOD_DATE FROM course_period_var cpv,college_periods sp WHERE cpv.ID=' . $period['ID'] . ' AND cpv.PERIOD_ID=sp.PERIOD_ID'));
...
foreach ($_openSIS['Menu'] as $modcat => $programs) {
    if (count($_openSIS['Menu'][$modcat])) {
        ...
        if (User('PROFILE') != 'admin' && $modcat == "collegesetup") {
            echo "<li " . (($current_mod == $modcat) ? 'class="active"' : '') . "><a HREF=javascript:void(0)><i class=\"{$menu_icons[$modcat]}\"></i><span>College Info</span></a>";
            ...
            } elseif ($modcat == 'collegesetup')
                echo "<li " . (($current_mod == $modcat) ? 'class="active"' : '') . "><a HREF=javascript:void(0)><i class=\"{$menu_icons[$modcat]}\"></i><span>College Setup</span></a>";
            else {
                    ...
                    if (User('PROFILE') == 'student' && $title == "Student Info") {
                        ...
                        if ($modcat == 'eligibility')
                            ...
                            if (User('PROFILE_ID') != 0 && User('PROFILE') == 'admin') {
                                ...
                                if ($modcat != 'tools')
                                    echo "<li  " . (($current_menu == $title) ? 'class="current-submenu"' : '') . "><A id=hm HREF=javascript:void(0) onClick='check_content(\"Ajax.php?modname=" . $file . " \");'  onmousedown='document.getElementById(\"header\").innerHTML = \"" . ($modcat == 'collegesetup' ? 'College Setup' : ucwords($modcat)) . " <i class=\"icon-arrow-right5\"></i> " . "$title\"' onmouseup=\"document.getElementById('cframe').src='Bottom.php?modname=" . str_replace('&', '?', $file) . "';\">$title</A>";
                            } else
                                echo "<li  " . (($current_menu == $title) ? 'class="current-submenu"' : '') . "><A id=hm HREF=javascript:void(0) onClick='check_content(\"Ajax.php?modname=" . $file . " \");'  onmousedown='$(\"#header\").html(\"" . ($modcat == 'collegesetup' ? 'College Setup' : ucwords($modcat)) . " <i class=icon-arrow-right5></i> " . $title."\");' onmouseup=\"$('#cframe').attr('src','Bottom.php?modname=" . str_replace('&', '?', $file) . "');\">$title</A>";
                        }
                    }
                    ...
                    if ($modcat == 'eligibility')
                        ...
                        if (User('PROFILE_ID') != 0 && User('PROFILE') == 'admin') {
                            ...
                            if ($modcat != 'tools') {
                                ...
                                echo "<li " . (($current_menu == $title) ? 'class="current-submenu"' : '') . "><A id=dd HREF=javascript:void(0) onClick='check_content(\"Ajax.php?modname=" . $file . " \");'  onmousedown='document.getElementById(\"header\").innerHTML = \"" . ($modcat == 'collegesetup' ? 'College Setup' : ucwords($modcat)) . " <i class=\"icon-arrow-right5\"></i> " . "$title\"' onmouseup=\"document.getElementById('cframe').src='Bottom.php?modname=" . $file . "';\">$title</A>";
                            }
                        } else
                            echo "<li " . (($current_menu == $title) ? 'class="current-submenu"' : '') . "><A id=dd HREF=javascript:void(0) onClick='check_content(\"Ajax.php?modname=" . $file . " \");'  onmousedown='document.getElementById(\"header\").innerHTML = \"" . ($modcat == 'collegesetup' ? 'College Setup' : ucwords($modcat)) . " <i class=\"icon-arrow-right5\"></i> " . "$title\"' onmouseup=\"document.getElementById('cframe').src='Bottom.php?modname=" . $file . "';\">$title</A>";
                    }
...
if ($_REQUEST['modname'] || $_GET['modname']) {
    ...
    if (optional_param('modname', '', PARAM_NOTAGS) == 'ParentLookup.php')
        $allowed = true;
    if (optional_param('modname', '', PARAM_NOTAGS) == 'collegesetup/UploadLogo.php' && User('PROFILE') == 'admin')
        $allowed = true;
