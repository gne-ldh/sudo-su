...
$calendars_RET = DBGet(DBQuery('SELECT CALENDAR_ID,ROLLOVER_ID FROM college_calendars WHERE COLLEGE_ID=\''.UserCollege().'\' AND SYEAR=\''.$next_syear.'\''));
		...
		case 'student_enrollment':
                                                    ...
                                                    DBQuery('INSERT INTO student_enrollment (SYEAR,NEXT_COLLEGE,COLLEGE_ID,STUDENT_ID,GRADE_ID,START_DATE,END_DATE,ENROLLMENT_CODE,DROP_CODE,CALENDAR_ID,LAST_COLLEGE) SELECT SYEAR+1,NEXT_COLLEGE,COLLEGE_ID,STUDENT_ID,(SELECT NEXT_GRADE_ID FROM college_gradelevels g WHERE g.ID=e.GRADE_ID),\''.$next_start_date.'\' AS START_DATE,NULL AS END_DATE,(SELECT ID FROM student_enrollment_codes WHERE SYEAR=\''.$next_syear.'\' AND TYPE=\'Roll\') AS ENROLLMENT_CODE,NULL AS DROP_CODE,(SELECT CALENDAR_ID FROM college_calendars WHERE ROLLOVER_ID=e.CALENDAR_ID),COLLEGE_ID FROM student_enrollment e WHERE e.SYEAR=\''.UserSyear().'\' AND e.COLLEGE_ID=\''.UserCollege().'\' AND ((\''.DBDate('mysql').'\' BETWEEN e.START_DATE AND e.END_DATE OR e.END_DATE IS NULL) AND \''.DBDate('mysql').'\'>=e.START_DATE) AND e.NEXT_COLLEGE=\''.UserCollege().'\'');
			// ROLL STUDENTS WHO ARE TO BE RETAINED
                                                    DBQuery('INSERT INTO student_enrollment (SYEAR,NEXT_COLLEGE,COLLEGE_ID,STUDENT_ID,GRADE_ID,START_DATE,END_DATE,ENROLLMENT_CODE,DROP_CODE,CALENDAR_ID,LAST_COLLEGE) SELECT SYEAR+1,NEXT_COLLEGE,COLLEGE_ID,STUDENT_ID,GRADE_ID,\''.$next_start_date.'\' AS START_DATE,NULL AS END_DATE,(SELECT ID FROM student_enrollment_codes WHERE SYEAR=\''.$next_syear.'\' AND TYPE=\'Roll\') AS ENROLLMENT_CODE,NULL AS DROP_CODE,(SELECT CALENDAR_ID FROM college_calendars WHERE ROLLOVER_ID=e.CALENDAR_ID),COLLEGE_ID FROM student_enrollment e WHERE e.SYEAR=\''.UserSyear().'\' AND e.COLLEGE_ID=\''.UserCollege().'\' AND ((\''.DBDate('mysql').'\' BETWEEN e.START_DATE AND e.END_DATE OR e.END_DATE IS NULL) AND \''.DBDate('mysql').'\'>=e.START_DATE) AND e.NEXT_COLLEGE=\'0\'');
			// ROLL STUDENTS TO NEXT COLLEGE
                                                    DBQuery('INSERT INTO student_enrollment (SYEAR,COLLEGE_ID,GRADE_ID,STUDENT_ID,START_DATE,END_DATE,NEXT_COLLEGE,ENROLLMENT_CODE,DROP_CODE,CALENDAR_ID,LAST_COLLEGE) SELECT SYEAR+1,NEXT_COLLEGE,(SELECT g.ID FROM college_gradelevels g WHERE g.SORT_ORDER=1 AND g.COLLEGE_ID=e.NEXT_COLLEGE),STUDENT_ID,\''.$next_start_date.'\' AS START_DATE,NULL AS END_DATE,NEXT_COLLEGE,(SELECT ID FROM student_enrollment_codes WHERE SYEAR=\''.$next_syear.'\' AND TYPE=\'Roll\') AS ENROLLMENT_CODE,NULL AS DROP_CODE,NULL,NEXT_COLLEGE FROM student_enrollment e WHERE e.SYEAR=\''.UserSyear().'\' AND e.COLLEGE_ID=\''.UserCollege().'\' AND ((\''.DBDate('mysql').'\' BETWEEN e.START_DATE AND e.END_DATE OR e.END_DATE IS NULL) AND \''.DBDate('mysql').'\'>=e.START_DATE) AND e.NEXT_COLLEGE NOT IN (\''.UserCollege().'\',\'0\',\'-1\')');
                                                    
                                                    DBQuery('INSERT INTO medical_info (STUDENT_ID,SYEAR,COLLEGE_ID,PHYSICIAN,PHYSICIAN_PHONE,PREFERRED_HOSPITAL) SELECT STUDENT_ID,\''.$next_syear.'\' as SYEAR,COLLEGE_ID,PHYSICIAN,PHYSICIAN_PHONE,PREFERRED_HOSPITAL FROM medical_info WHERE SYEAR='.UserSyear().' AND COLLEGE_ID='.UserCollege());
