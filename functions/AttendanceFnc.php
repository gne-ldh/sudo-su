...
#
#***************************************************************************************
function UpdateAttendanceDaily($college_roll_no,$date='',$comment=false)
{
	if(!$date)
		$date = DBDate();
	...
	$current_mp=GetCurrentMP('QTR',$date);
        ...
        if(!$current_mp){
			...
			WHERE
				...
				AND ac.COLLEGE_DATE=\''.$date.'\' AND (ac.BLOCK=sp.BLOCK OR sp.BLOCK IS NULL)
                                AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID
				AND ac.CALENDAR_ID=cp.CALENDAR_ID AND ac.COLLEGE_ID=s.COLLEGE_ID AND ac.SYEAR=s.SYEAR
				...
				AND s.COLLEGE_ROLL_NO=\''.$college_roll_no.'\'
	...
	if($total==0)
		return;
        $current_RET = DBGet(DBQuery('SELECT MINUTES_PRESENT,STATE_VALUE,COMMENT FROM attendance_day WHERE COLLEGE_ROLL_NO='.$college_roll_no.' AND COLLEGE_DATE=\''.$date.'\''));
        ...
        $sql = 'SELECT SUM(IF(cp.HALF_DAY LIKE \'Y\',(SELECT half_day_minute FROM system_preference WHERE college_id='.UserCollege().'),sp.LENGTH)) AS TOTAL
			...
			WHERE ap.COLLEGE_ROLL_NO=\''.$college_roll_no.'\' AND ap.COLLEGE_DATE=\''.$date.'\' AND ap.PERIOD_ID=sp.PERIOD_ID AND ac.ID = ap.ATTENDANCE_CODE AND ac.STATE_CODE=\'P\'
	...
	$sql = 'SELECT SUM(sp.LENGTH) AS TOTAL
			...
			WHERE ap.COLLEGE_ROLL_NO=\''.$college_roll_no.'\' AND ap.COLLEGE_DATE=\''.$date.'\' AND ap.PERIOD_ID=sp.PERIOD_ID AND ac.ID = ap.ATTENDANCE_CODE AND ac.STATE_CODE=\'H\'
	...
	$current_RET = DBGet(DBQuery('SELECT MINUTES_PRESENT,STATE_VALUE,COMMENT FROM attendance_day WHERE COLLEGE_ROLL_NO=\''.$college_roll_no.'\' AND COLLEGE_DATE=\''.$date.'\''));
		...
		DBQuery('UPDATE attendance_day SET STATE_VALUE=\''.$length.'\' WHERE COLLEGE_ROLL_NO=\''.$college_roll_no.'\' AND COLLEGE_DATE=\''.$date.'\'');
	if(count($current_RET) && $current_RET[1]['MINUTES_PRESENT']!=$total)
		DBQuery('UPDATE attendance_day SET MINUTES_PRESENT=\''.$total.'\',STATE_VALUE=\''.$length.'\''.($comment!=false?',COMMENT=\''.singleQuoteReplace("","",$comment).'\'':'').' WHERE COLLEGE_ROLL_NO=\''.$college_roll_no.'\' AND COLLEGE_DATE=\''.$date.'\'');
        elseif(count($current_RET) && $comment!=false && $current_RET[1]['COMMENT']!=$comment)
		DBQuery('UPDATE attendance_day SET COMMENT=\''.singleQuoteReplace("","",$comment).'\' WHERE COLLEGE_ROLL_NO=\''.$college_roll_no.'\' AND COLLEGE_DATE=\''.$date.'\'');
	elseif(count($current_RET)==0)
        {
                $check_assoc=DBGet(DBQuery('SELECT COUNT(*) as REC_EX FROM attendance_period ap,course_periods cp WHERE ap.COLLEGE_ROLL_NO='.$college_roll_no.' AND ap.COLLEGE_DATE=\''.$date.'\' AND cp.COURSE_PERIOD_ID=ap.COURSE_PERIOD_ID AND cp.COLLEGE_ID='.UserCollege().' AND cp.SYEAR='.UserSyear()));
                if($check_assoc[1]['REC_EX']>0)
                DBQuery('INSERT INTO attendance_day (SYEAR,COLLEGE_ROLL_NO,COLLEGE_DATE,MINUTES_PRESENT,STATE_VALUE,MARKING_PERIOD_ID,COMMENT) values(\''.UserSyear().'\',\''.$college_roll_no.'\',\''.$date.'\',\''.$total.'\',\''.$length.'\',\''.$current_mp.'\',\''.singleQuoteReplace("","",$comment).'\')');
        }
}
