...
{	global $profiles_RET;
        ...
        switch(User('PROFILE'))
	{
		...
		$profiles_RET = DBGet(DBQuery('SELECT * FROM user_profiles'),array(),array('ID'));
                ...
                FROM
					staff s INNER JOIN staff_college_relationship ssr USING(staff_id) '.$extra['FROM'].',login_authentication la
				WHERE
		...
		if($_REQUEST['_search_all_colleges']!='Y')
			$sql .= ' AND college_id='.  UserCollege().' ';
                   else
                       $sql .= ' AND college_id IN('.  GetUserColleges(UserID(),true).') ';
		if($_REQUEST['_dis_user']!='Y')
...
function GetStaffList_Miss_Atn(& $extra)
{
		...
		switch(User('PROFILE'))
		{
			...
			$sql = 'SELECT CONCAT(s.LAST_NAME, \' \', s.FIRST_NAME) AS FULL_NAME,
					...
					FROM
						staff s INNER JOIN staff_college_relationship ssr USING(staff_id) '.$extra['FROM'].'
					WHERE
						ssr.SYEAR=\''.UserSyear().'\'';
			if($_REQUEST['_search_all_colleges']!='Y')
				$sql .= ' AND ssr.college_id='.UserCollege().' ';
			if($_REQUEST['username'])
