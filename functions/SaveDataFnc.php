				...
				if($id=='new')
				{
					...
					if(strlen($value)>0){
					     ...
					     $up_values = $column.'=\''.singleQuoteReplace('','',$value).'\',';
                                             ...
                                             {
                                                $sc_qr=  DBGet(DBQuery('SELECT course_period_id from schedule  WHERE COLLEGE_ROLL_NO=\''.$_REQUEST['student_id'].'\' AND COLLEGE_ID=\''.UserCollege().'\'  AND SYEAR=\''.UserSyear().'\'  AND (END_DATE IS NULL OR \''.$value.'\' < END_DATE )'));
                                                ...
                                                foreach($sc_qr as $v)
                                                               ...
                                                               $sc_end_date=$sch_en[1]['END_DATE'];
                                                                ...
                                                                if(strtotime($value)<strtotime($sc_end_date))
                                                                       ...
                                                                       DBQuery('UPDATE schedule SET END_DATE=\''.$final_end_date.'\' WHERE COLLEGE_ROLL_NO=\''.$_REQUEST['student_id'].'\' AND COLLEGE_ID=\''.UserCollege().'\'  AND SYEAR=\''.UserSyear().'\' and course_period_id=\''.$v['COURSE_PERIOD_ID'].'\'');  
                                                                   }
			...
			if($id=='new')
				 $sql[$table] = 'INSERT INTO '.$table.' (' . $iu_extra['fields'][$table].substr($ins_fields[$table],0,-1) . ') values(' . $iu_extra['values'][$table].substr($ins_values[$table],0,-1) . ')';
			else
                        {
                            ...
                            if($table=='student_enrollment')
                            {
                                  $enrollment_record=DBGet(DBQuery("SELECT * FROM student_enrollment WHERE COLLEGE_ROLL_NO='$_REQUEST[student_id]' AND SYEAR='".UserSyear()."' AND COLLEGE_ID='".UserCollege()."'"));
