...
{	global $_openSIS;
	if($marking_period_id==0)
	{
		...
		$RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID FROM college_years WHERE SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''));
	...
	if(!$_openSIS['GetAllMP'][$mp])
	{
		switch($mp)
		{
			case 'PRO':
				...
				$RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,SEMESTER_ID FROM college_quarters WHERE SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''));
			...
			case 'SEM':
				...
				$RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,SEMESTER_ID FROM college_quarters WHERE SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''),array(),array('SEMESTER_ID'));
				...
				$RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID FROM college_semesters s WHERE NOT EXISTS (SELECT \'\' FROM college_quarters q WHERE q.SEMESTER_ID=s.MARKING_PERIOD_ID) AND SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''));
				foreach($RET as $value)
...
{	global $_openSIS;
	...
	if(!$_openSIS['GetParentMP'][$mp])
	{
		switch($mp)
		{
			...
			case 'SEM':
				$_openSIS['GetParentMP'][$mp] = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,SEMESTER_ID AS PARENT_ID FROM college_quarters WHERE SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''),array(),array('MARKING_PERIOD_ID'));
			break;

			case 'FY':
				$_openSIS['GetParentMP'][$mp] = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,YEAR_ID AS PARENT_ID FROM college_semesters WHERE SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''),array(),array('MARKING_PERIOD_ID'));
			break;
		}
	}
...
{	global $_openSIS;
	...
	switch($mp)
	{
		...
		case 'SEM':
			if(GetMP($marking_period_id,'TABLE')=='college_quarters')
		...
		case 'PRO':
			...
			{
				$RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,QUARTER_ID FROM college_progress_periods WHERE SYEAR=\''.UserSyear().'\' AND COLLEGE_ID=\''.UserCollege().'\''),array(),array('QUARTER_ID'));
