...
$extra['SELECT'] .= ',smc.COMMENT AS STUDENT_MP_COMMENT,rpg.TITLE as GRADE_TITLE,sg1.COLLEGE_ROLL_NO,sg1.COURSE_PERIOD_ID,sg1.MARKING_PERIOD_ID,sg1.COMMENT,c.TITLE as COURSE_TITLE,
...
$extra['FROM'] .= ',student_report_card_grades sg1 LEFT OUTER JOIN report_card_grades rpg ON (rpg.ID=sg1.REPORT_CARD_GRADE_ID) LEFT OUTER JOIN student_mp_comments smc ON (smc.COLLEGE_ROLL_NO=sg1.COLLEGE_ROLL_NO AND smc.MARKING_PERIOD_ID=sg1.MARKING_PERIOD_ID),
                    courses c,course_periods cp1,college_periods sp';
$extra['WHERE'] .= ' AND sg1.MARKING_PERIOD_ID IN ('.$mp_list.')
                    AND cp1.COURSE_PERIOD_ID=sg1.COURSE_PERIOD_ID AND c.COURSE_ID = cp1.COURSE_ID AND sg1.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO AND sp.PERIOD_ID=cp1.PERIOD_ID';
...
else
{
	...
	$extra['WHERE'] .= ' AND ad.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO AND ad.SYEAR=ssm.SYEAR AND ad.STATE_VALUE=\'0.0\'';
	$extra['group'][] = 'COLLEGE_ROLL_NO';
	$extra['group'][] = 'MARKING_PERIOD_ID';
	Widgets('course');
	Widgets('gpa');
	...
	{	
		foreach($RET as $college_roll_no=>$course_periods)
		{
			...
			$this_PDF = str_replace('/T(values[students][COLLEGE_ROLL_NO])',"/T(values[$student_count][students][COLLEGE_ROLL_NO])",$this_PDF);
			$this_PDF = str_replace('(default[students][COLLEGE_ROLL_NO])','('.$course_periods[$course_period_id][key($course_periods[$course_period_id])][1]['COLLEGE_ROLL_NO'].')',$this_PDF);
