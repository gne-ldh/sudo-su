...
if ($_REQUEST['modfunc'] == 'save' && $_REQUEST['honor_roll']) {
    if (count($_REQUEST['st_arr'])) {
        ...
        if ($_REQUEST['honor_roll'] != 986) {
            ...
            $scale = $COLLEGE_RET[1]['REPORTING_GP_SCALE'];
            $honor = DBGet(DBQuery('SELECT VALUE  FROM honor_roll WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND SYEAR=\'' . UserSyear() . '\' ORDER BY VALUE DESC'));
            ...
            if ($_REQUEST['w_course_period_id']) {
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE  hr.COLLEGE_ID=' . UserCollege() . ' AND hr.SYEAR=' . UserSyear() . ' AND  hr.VALUE=(SELECT if((ROUND(AVG(srcg.grade_percent))>=' . $honor_gpa1 . ' and ROUND(AVG(srcg.grade_percent))<' . $honor_gpa2 . '),' . $honor_gpa1 . ',"")  FROM
                                                   ...
                                                   and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO) )AS HONOR_ROLL';
                $extra['WHERE'] .= 'AND ((SELECT ROUND(AVG(srcg.grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y"
                                                   and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)>=' . $honor_gpa1 . ' ) AND ((SELECT ROUND(AVG(grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y"
                                                   and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)<' . $honor_gpa2 . ' )  GROUP BY s.COLLEGE_ROLL_NO';
                $extra['SELECT'] .= ',(SELECT CONCAT(st.LAST_NAME,", ",coalesce(st.FIRST_NAME)) FROM staff st,course_periods cp,course_period_var cpv,college_periods p,schedule ss WHERE st.STAFF_ID=cp.TEACHER_ID AND cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND  cp.COURSE_PERIOD_ID=\'' . $_REQUEST['w_course_period_id'] . '\' AND ss.COLLEGE_ROLL_NO=s.COLLEGE_ROLL_NO AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date('Y-m-d', strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS TEACHER';
                $extra['SELECT'] .= ',(SELECT cpv.ROOM_ID AS ROOM FROM course_periods cp,course_period_var cpv,college_periods p,schedule ss WHERE cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND  cp.COURSE_PERIOD_ID=\'' . $_REQUEST['w_course_period_id'] . '\' AND ss.COLLEGE_ROLL_NO=s.COLLEGE_ROLL_NO AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date("Y-m-d", strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS ROOM';
            } else {
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE  hr.VALUE=(SELECT if((ROUND(AVG(srcg.grade_percent))>=' . $honor_gpa1 . ' and ROUND(AVG(srcg.grade_percent))<' . $honor_gpa2 . '),' . $honor_gpa1 . ',"")  FROM
                                                   ...
                                                   and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)  AND hr.COLLEGE_ID=' . UserCollege() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                $extra['WHERE'] .= 'AND ((SELECT ROUND(AVG(srcg.grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y"
                                                   and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)>=' . $honor_gpa1 . ' ) AND ((SELECT ROUND(AVG(grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y"
                                                   and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)<' . $honor_gpa2 . ' )  GROUP BY s.COLLEGE_ROLL_NO';
                $extra['SELECT'] .= ',(SELECT CONCAT(st.LAST_NAME,", ",coalesce(st.FIRST_NAME)) FROM staff st,course_periods cp,course_period_var cpv,college_periods p,schedule ss WHERE st.STAFF_ID=cp.TEACHER_ID AND cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.COLLEGE_ROLL_NO=s.COLLEGE_ROLL_NO AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date('Y-m-d', strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS TEACHER';
                $extra['SELECT'] .= ',(SELECT cpv.ROOM_ID AS ROOM FROM course_periods cp,course_period_var cpv,college_periods p,schedule ss WHERE cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.COLLEGE_ROLL_NO=s.COLLEGE_ROLL_NO AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date("Y-m-d", strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS ROOM';
            }
            ...
            if ($_REQUEST['w_course_period_id']) {
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                ...
                                                                (SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y" and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO) order by hr.value desc limit 1)
                                                                ...
                                                                and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO) order by hr.value asc limit 1)),(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE>(SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y"
                                                                and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO) order by hr.value asc limit 1),(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE<=(SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y"
                                                                and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO) order by hr.value desc limit 1))
                                                                FROM `student_report_card_grades`srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y" and `COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)  AND hr.COLLEGE_ID=' . UserCollege() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                ...
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                (SELECT if((ROUND(AVG(srcg.grade_percent))>=
                                                                (SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE>=
                                                                (SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y" and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO) order by hr.value desc limit 1)
                                                                and ROUND(AVG(srcg.grade_percent))<
                                                                (SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE
                                                                ...
                                                                and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO) order by hr.value asc limit 1),(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE<=(SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y"
                                                                and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO) order by hr.value desc limit 1))
                                                                FROM `student_report_card_grades`srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y" and `COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)  AND hr.COLLEGE_ID=' . UserCollege() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                $extra['SELECT'] .= ',(SELECT CONCAT(st.LAST_NAME,\', \',coalesce(st.FIRST_NAME)) FROM staff st,course_periods cp,course_period_var cpv,college_periods p,schedule ss WHERE st.STAFF_ID=cp.TEACHER_ID AND cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE=\'Y\' AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.COLLEGE_ROLL_NO=s.COLLEGE_ROLL_NO AND ss.SYEAR=\'' . UserSyear() . '\' AND ss.MARKING_PERIOD_ID = \'' . UserMp() . '\' AND (ss.START_DATE<=\'' . DBDate() . '\' AND (ss.END_DATE>=\'' . DBDate() . '\' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS TEACHER';
                $extra['SELECT'] .= ',(SELECT cpv.ROOM_ID AS ROOM FROM course_periods cp,course_period_var cpv,college_periods p,schedule ss WHERE cpv.PERIOD_id=p.PERIOD_ID AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND p.ATTENDANCE=\'Y\' AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.COLLEGE_ROLL_NO=s.COLLEGE_ROLL_NO AND ss.SYEAR=\'' . UserSyear() . '\' AND ss.MARKING_PERIOD_ID = \'' . UserMp() . '\' AND (ss.START_DATE<=\'' . DBDate() . '\' AND (ss.END_DATE>=\'' . DBDate() . '\' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS ROOM';
            }
            $extra['ORDER_BY'] = 'HONOR_ROLL,SORT_ORDER DESC,ROOM,FULL_NAME';
        ...
        if ($_REQUEST['list']) {
            ...
            $columns = array('FULL_NAME' => 'Student', 'COLLEGE_ROLL_NO' => 'College Roll No', 'ALT_ID' => 'Alternate ID', 'GRADE_ID' => 'Grade', 'PHONE' => 'Phone', 'HONOR_ROLL' => 'Honor Roll');
...
function MyWidgets($item, $mp) {
    ...
    switch ($item) {
        case 'honor_roll':
            ...
            if ($_REQUEST['honor_roll'] != 986 && $_REQUEST['honor_roll']) {
                ...
                if ($honor_gpa2) {
                    ...
                    if ($_REQUEST['w_course_period_id']) {
                        ...
                        $extra['WHERE'] .= 'AND ((SELECT ROUND(AVG(srcg.grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll=\'Y\'
                                                   and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO)>=' . $honor_gpa1 . ' ) AND ((SELECT ROUND(AVG(srcg.grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll=\'Y\'
                                                   and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO)<' . $honor_gpa2 . ' )  ';
                    } else {
                        $extra['SELECT'] .= ',( SELECT hr.TITLE FROM honor_roll hr WHERE  hr.VALUE=(SELECT if((ROUND(AVG(srcg.grade_percent))>=' . $honor_gpa1 . ' and ROUND(AVG(srcg.grade_percent))<' . $honor_gpa2 . '),' . $honor_gpa1 . ',"")  FROM
                                                   `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll=\'Y\'
                                                   and srcg.COLLEGE_ROLL_NO=ssm.COLLEGE_ROLL_NO)  AND hr.COLLEGE_ID=' . UserCollege() . '  AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                ...
                if ($_REQUEST['w_course_period_id'])
                    $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                ...
                                                                and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO) order by hr.value asc limit 1),(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE<=(SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll=\'Y\'
                                                                and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO) order by hr.value desc limit 1))
                                                                FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y" and srcg.`COLLEGE_ROLL_NO`=ssm.COLLEGE_ROLL_NO)  AND hr.COLLEGE_ID=' . UserCollege() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                else
                    $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                (SELECT if(
                                                                            (ROUND(AVG(srcg.grade_percent))>=(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE>=
            ...
            $option = DBGet(DBQuery('SELECT TITLE,VALUE  FROM honor_roll WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND SYEAR=\'' . UserSyear() . '\'  ORDER BY VALUE'));
            $options['986'] = 'All';
