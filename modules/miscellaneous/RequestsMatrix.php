...
include('../../RedirectModulesInc.php');
	$requests_RET = DBGet(DBQuery("SELECT concat(r.COURSE_ID, '_', r.COURSE_WEIGHT) AS CRS,
                                        ...
                                        (cp.TOTAL_SEATS-cp.FILLED_SEATS) AS OPEN_SEATS,s.COLLEGE_ROLL_NO AS SCHEDULED
                                        FROM schedule_requests r,
                                        courses c,college_periods sp,
                                        course_periods cp LEFT OUTER JOIN schedule s ON 
                                        (s.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND s.COLLEGE_ROLL_NO='".UserStudentID()."')
                                        WHERE 
                                        r.SYEAR='".UserSyear()."' AND r.COLLEGE_ID='".UserCollege()."'
                                        AND r.COURSE_ID=cp.COURSE_ID AND c.COURSE_ID=cp.COURSE_ID
                                        AND r.COLLEGE_ROLL_NO='".UserStudentID()."'
                                        AND sp.PERIOD_ID=cp.PERIOD_ID
                                        ORDER BY ".db_case(array('s.COLLEGE_ROLL_NO',"''","NULL",'sp.SORT_ORDER'))."
