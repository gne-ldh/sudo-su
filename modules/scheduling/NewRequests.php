...
if($_REQUEST['modfunc']=='verify')
{
	...
	DBQuery('DELETE FROM schedule_requests WHERE COLLEGE_ROLL_NO=\''.UserStudentID().'\' AND SYEAR=\''.UserSyear().'\'');
...
if($_REQUEST['modfunc']=='choose')
{
	...
	$requests_RET = DBGet(DBQuery('SELECT sr.COURSE_ID,c.COURSE_TITLE,sr.WITH_PERIOD_ID,sr.NOT_PERIOD_ID,sr.WITH_TEACHER_ID,
									...
									WHERE sr.SYEAR=\''.UserSyear().'\' AND sr.COLLEGE_ROLL_NO=\''.UserStudentID().'\' AND sr.COURSE_ID=c.COURSE_ID'),$functions);
