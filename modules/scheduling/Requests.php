...
function _makePeriod($value, $column) {
    ...
    $periods_RET = DBGet(DBQuery('SELECT p.TITLE,p.PERIOD_ID FROM college_periods p,course_periods cp,course_period_var cpv WHERE p.PERIOD_ID=cpv.PERIOD_ID AND cp.COURSE_ID=\'' . $THIS_RET['COURSE_ID'] . '\' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID'));
