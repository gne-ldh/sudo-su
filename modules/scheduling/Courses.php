		...
		foreach($tables as $id=>$columns)
		{
			...
			else
			{
				...
				elseif($table_name=='course_periods')
				{
					...
					$values = '\''.UserSyear().'\',\''.UserCollege().'\','.$_REQUEST[course_id].','.$title.',';
...
if((!$_REQUEST['modfunc'] || $_REQUEST['modfunc']=='choose_course') && !$_REQUEST['course_modfunc'])
{
	...
	if($_REQUEST['modfunc']!='choose_course')
	{
		...
		if($_REQUEST['course_period_id'])
		{
			...
			$teachers_RET = DBGet(DBQuery('SELECT STAFF_ID,LAST_NAME,FIRST_NAME,MIDDLE_NAME FROM staff WHERE (COLLEGES IS NULL OR FIND_IN_SET(\''.UserCollege().'\',COLLEGES)>0) AND SYEAR=\''.UserSyear().'\' AND PROFILE='.'teacher'.' ORDER BY LAST_NAME,FIRST_NAME'));
