...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='save' && AllowEdit())
{
	...
	foreach($_REQUEST['student'] as $student_id=>$yes)
	{
		...
		{
			$sql = 'INSERT INTO students_join_users (COLLEGE_ROLL_NO,STAFF_ID) values(\''.$student_id.'\',\''.UserStaffID().'\')';
			DBQuery($sql);
		}
	}
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='delete' && AllowEdit())
{
	...
	{
		DBQuery('DELETE FROM students_join_users WHERE COLLEGE_ROLL_NO=\''.$_REQUEST[student_id].'\' AND STAFF_ID=\''.UserStaffID().'\'');
		unset($_REQUEST['modfunc']);
	}
}
...
//			echo '<script language=JavaScript>parent.side.location="'.$_SESSION['Side_PHP_SELF'].'?modcat="+parent.side.document.forms[0].modcat.value;</script>';			
	...
	if(UserStaffID())
	{
		...
		$link['remove'] = array('link'=>"Modules.php?modname=$_REQUEST[modname]&modfunc=delete",'variables'=>array('student_id'=>'COLLEGE_ROLL_NO'));
