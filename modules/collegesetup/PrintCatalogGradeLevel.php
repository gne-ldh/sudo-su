...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'print' && $_REQUEST['report']) {
    ...
    $sql = 'select distinct
				...
				from course_periods cp' . $from . ' where cp.college_id=\'' . UserCollege() . '\' and cp.syear=\'' . UserSyear() . '\' ' . $where . ' order by subject,COURSE_TITLE';
    ...
    if (count($ret)) {
        ...
        foreach ($ret as $s_id) {
            ...
            $grade_level_RET = DBGet(DBQuery('SELECT TITLE FROM college_gradelevels WHERE id=\'' . $_REQUEST['id'] . '\''));
            $grade_title = $grade_level_RET[1]['TITLE'];

            if ($grade_title != '') {
                echo "<tr><td width=105>" . DrawLogo() . "</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">" . GetCollege(UserCollege()) . "<div style=\"font-size:12px;\">Course catalog by Grade Level : " . $grade_title . "</div></td><td align=right style=\"padding-top:20px;\">" . ProperDate(DBDate()) . "<br />Powered by openSIS</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
            } else {
...
function _makeDetails($value, $coulmn) {
    ...
    switch ($coulmn) {
        ...
        case 'PERIOD':
            $period_title = array();
            foreach ($get_dt as $gd) {
                $title = DBGet(DBQuery('SELECT TITLE FROM college_periods WHERE PERIOD_ID=' . $gd['PERIOD_ID']));
