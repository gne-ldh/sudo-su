...
if ($_REQUEST['course_period_id'] != 'new') {
    if ($_REQUEST['tables']['course_periods'][$_REQUEST['course_period_id']]['SCHEDULE_TYPE'] == 'FIXED') { // echo 'sss';
        ...
        if ($get_seat[1]['CAPACITY'] < $total_seats) {
            ...
            echo '<font color=red>Total seats cannot be greater than room capacity. Change the capacity in College Setup-> Rooms</font>';
        }
    }
        ...
        if (DeletePromptCommon('course period')) {
            ...
            if ($checking_days1[1]['TOTAL'] > 1) {
                ...
                foreach ($data_RET as $count => $data) {
                    if ($data['PERIOD_ID'] != '') {
                        $period = '';
                        $qry = "SELECT short_name FROM college_periods WHERE period_id=$data[PERIOD_ID]";
                ...
                foreach ($days as $day => $period) {
                    if ($day != 'n') {
                        ...
                        if (isset($period['PERIOD_ID']) && !isset($period['START_TIME']) && !isset($period['END_TIME']) && $period['PERIOD_ID'] != '') {
                            ...
                            $period_data = DBGet(DBQuery('SELECT START_TIME,END_TIME FROM college_periods WHERE PERIOD_ID=' . $period['PERIOD_ID'] . ''));
...
//    print_r($_SESSION);exit;
    ...
    $college_fy_dates = DBGet(DBQuery('select start_date,end_date from college_years where syear = ' . UserSyear() . ' and college_id = ' . UserCollege()));
    $college_fy_bdate = $college_fy_dates[1]['START_DATE'];
    $college_fy_edate = $college_fy_dates[1]['END_DATE'];
        ...
        foreach ($tables as $id => $columns) {
            ...
            if (!(attendanceAssociation($id))) {
                ...
                if ($schedule_type[1]['SCHEDULE_TYPE'] == 'FIXED') {
                    $check_attn = DBGet(DBQuery('SELECT * FROM college_periods WHERE PERIOD_ID=(SELECT PERIOD_ID FROM course_period_var WHERE COURSE_PERIOD_ID=' . $id . ')'));
...
//                            
//                            }
//                                      if (GetMP($columns['MARKING_PERIOD_ID'], 'TABLE') != 'college_years')
//                                $mp_title = GetMP($columns['MARKING_PERIOD_ID'], 'SHORT_NAME') . ' - ';
//                        } else {
//                            if (GetMP($current[1]['MARKING_PERIOD_ID'], 'TABLE') == 'college_years')
//                                $current[1]['MP'] = 'FY';
//                            elseif (GetMP($current[1]['MARKING_PERIOD_ID'], 'TABLE') == 'college_semesters')
//                                $current[1]['MP'] = 'SEM';
//                            else
//                                $current[1]['MP'] = 'QTR';
//
...
//                        }
            if (isset($columns['BEGIN_DATE']) && $columns['BEGIN_DATE'] != '' && isset($columns['END_DATE']) && $columns['END_DATE'] != '') {
                if (strtotime($columns['BEGIN_DATE']) < strtotime($college_fy_bdate)) {
                    echo '<font color=red><b>Begin date cannot be less than full year begin date.</b></font>';
                    $not_pass_update = true;
                    break 2;
                }

                if (strtotime($columns['END_DATE']) > strtotime($college_fy_edate)) {
            ...
            if ($columns['MARKING_PERIOD_ID'] != '') {
                $mark_id_dates = DBGet(DBQuery('SELECT * FROM marking_periods WHERE COLLEGE_ID=' . UserCollege() . ' AND SYEAR=' . UserSyear() . ' AND MARKING_PERIOD_ID=' . $columns['MARKING_PERIOD_ID'] . ''));
                $columns['BEGIN_DATE'] = $mark_id_dates[1]['START_DATE'];
                $columns['END_DATE'] = $mark_id_dates[1]['END_DATE'];
                if (GetMP($columns['MARKING_PERIOD_ID'], 'TABLE') == 'college_years')
                    $columns['MP'] = 'FY';
                elseif (GetMP($columns['MARKING_PERIOD_ID'], 'TABLE') == 'college_semesters')
                    $columns['MP'] = 'SEM';
                elseif (GetMP($columns['MARKING_PERIOD_ID'], 'TABLE') == 'college_quarters')
                    $columns['MP'] = 'QTR';
            }
            else {
                ...
                if ($table_name == 'course_periods') {
                    ...
                    if ($columns['SCHEDULE_TYPE'] == 'VARIABLE') {
                        ...
                        foreach ($title_val as $key => $title_value) {
                            ...
                            $pd_id = DBGet(DBQuery("SELECT SHORT_NAME FROM college_periods WHERE PERIOD_ID='" . $title_value['PERIOD_ID'] . "'"));
                    ...
                    if ($columns['SCHEDULE_TYPE'] == 'FIXED') {
                        ...
                        $pd_id = DBGet(DBQuery("SELECT SHORT_NAME FROM college_periods WHERE PERIOD_ID='" . $title_val[1]['PERIOD_ID'] . "'"));
                    ...
                    if ($columns['SCHEDULE_TYPE'] == 'BLOCKED') {
                        ...
                        if (isset($columns['CALENDAR_ID']) && ($columns['CALENDAR_ID'] != $values[1]['CALENDAR_ID'])) {
                            ...
                            foreach ($cal_days as $cal_days_k => $cal_days_v) {
                                ...
                                $cal_days_new[] = $cal_days_v['COLLEGE_DATE'];
                            }
...
//                                    if (($table_name == 'course_periods' || $table_name == 'course_period_var') && ($column == 'COURSE_WEIGHT' || $column == 'GRADE_SCALE_ID' || $column == 'CREDITS' || $column == 'DOES_BREAKOFF' || $column == 'DOES_HONOR_ROLL' || $column == 'HALF_DAY' || $column == 'DOES_CLASS_RANK' || $column == 'SECONDARY_TEACHER_ID') && !gradeAssociation($id)) {
                                ...
                                if ($column == 'DOES_BREAKOFF' && $table_name == 'course_periods') {
                                    $sql1 .= $column . " = NULL,";
                                    $go = true;
                                }
                                ...
                                if ($column == 'SHORT_NAME' || $column == 'TOTAL_SEATS' || $column == 'DOES_ATTENDANCE' || $column == 'DOES_BREAKOFF'  || $column == 'CREDITS') {
                                    ...
                                    if ($column == 'DOES_ATTENDANCE') {
                                        if (scheduleAssociation($id)) {
                                            ...
                                            if ($columns['PERIOD_ID'] != '') {
                                                $check_attn = DBGet(DBQuery('SELECT * FROM college_periods WHERE PERIOD_ID=' . $columns['PERIOD_ID']));
                                                $check_attn = $check_attn[1];
                                            } else {
                                                $check_attn = DBGet(DBQuery('SELECT * FROM college_periods WHERE PERIOD_ID=(SELECT PERIOD_ID FROM course_period_var WHERE COURSE_PERIOD_ID=' . $id . ')'));
                                                $check_attn = $check_attn[1];
...
//                                            if (gradeAssociation($id) && ($column == 'TEACHER_ID' || $column == 'SECONDARY_TEACHER_ID' || $column == 'GRADE_SCALE_ID' || $column == 'CREDITS' || $column == 'DOES_BREAKOFF' || $column == 'DOES_HONOR_ROLL' || $column == 'HALF_DAY' || $column == 'DOES_CLASS_RANK')) {
                                        ...
                                        if (!scheduleAssociation($id) && $table_name == 'course_period_var' && $_REQUEST['tables']['course_periods'][$id]['SCHEDULE_TYPE'] == 'FIXED') {
                                            ...
                                            if ($column == 'PERIOD_ID') {
                                                $period_time_edt = DBGet(DBQuery("SELECT START_TIME, END_TIME FROM college_periods WHERE period_id={$value}"));
                                                $period_time_edt = $period_time_edt[1];
                                                $start_time_edt = $period_time_edt['START_TIME'];
                                                $end_time_edt = $period_time_edt['END_TIME'];
                                                $check_attn = DBGet(DBQuery('SELECT * FROM college_periods WHERE PERIOD_ID=' . $value));
                                                $check_attn = $check_attn[1];
                                                if ($check_attn['ATTENDANCE'] == 'Y')
...
//                                echo "<font color=red><b>Cannot Modify this course period as it has association. </b></font>";
                ...
                if ($table_name == 'course_subjects') {
                    ...
                    if ($columns['SCHEDULE_TYPE'] == 'FIXED') {
                        ...
                        $pd_id = DBGet(DBQuery("SELECT * FROM college_periods WHERE PERIOD_ID='" . $_REQUEST['tables']['course_period_var']['new']['PERIOD_ID'] . "'"));
                        $period_value.=$pd_id[1]['SHORT_NAME'];
                        ...
                        $pd_id = DBGet(DBQuery("SELECT SHORT_NAME FROM college_periods WHERE PERIOD_ID='" . $_REQUEST['course_period_variable']['new']['PERIOD_ID'] . "'"));
                        $period_value.=$pd_id[1]['SHORT_NAME'];
                    ...
                    $id[1]['ID'] = $id[1]['AUTO_INCREMENT'];
                    $fields = 'SYEAR,COLLEGE_ID,COURSE_ID,TITLE,MODIFIED_BY,';
                    ...
                    $title = str_replace("'", "''", str_replace("\'", "'", $mp_title) . $teacher[1]['FIRST_NAME'] . ' ' . $teacher[1]['MIDDLE_NAME'] . ' ' . $teacher[1]['LAST_NAME']);
                    $values = "'" . UserSyear() . "','" . UserCollege() . "','$_REQUEST[course_id]','$title'," . User('STAFF_ID') . ",";
...
//
//                                $pd_id = DBGet(DBQuery("SELECT SHORT_NAME FROM college_periods WHERE PERIOD_ID='" . $cpv_det['PERIOD_ID'] . "'"));
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'delete' && AllowEdit()) {
    ...
    if (clean_param($_REQUEST['course_period_id'], PARAM_ALPHANUM)) {
        ...
        if ($course1['COURSE_PERIOD_ID'] != '') {
            ...
            echo '<div align=right><a href=Modules.php?modname=collegesetup/Courses.php&subject_id=' . $subject_id . '&course_id=' . $course_id . ' style="text-decoration:none"><b>back to Course</b></a></div>';
            PopTable('footer');
        } else {
            if (DeletePromptCommon($table)) {
                if (BlockDelete($table)) {
        ...
        if ($subject1['COURSE_ID'] != '') {
            ...
            echo '<div align=right><a href=Modules.php?modname=collegesetup/Courses.php&subject_id=' . $subject_id . ' style="text-decoration:none"><b>back to Subject</b></a></div>';
            PopTable('footer');
        } else {
            if (DeletePromptCommon($table)) {
                if (BlockDelete($table)) {
...
//print_r($_REQUEST);
    ...
    if ($_POST['button'] == 'Save' && $conflict === true) {
        ...
        if ($_REQUEST['mode'] == 'add') {
            ...
            if ($chek_assoc[1]['REC_EX'] == 0) {
                if ($_REQUEST['values']['PERIOD_ID'] != '' && $_REQUEST['values']['DOES_ATTENDANCE'] == '') {
                    $check_sp = DBGet(DBQuery('SELECT * FROM college_periods WHERE PERIOD_ID=' . $_REQUEST['values']['PERIOD_ID']));
                    $check_sp = $check_sp[1];
                    ...
                    $cp_per_id = DBGet(DBQuery('SELECT * FROM course_period_var WHERE ID=' . $_REQUEST['cpv_id']));
                    $cp_per_id = $cp_per_id[1];
                    $check_sp = DBGet(DBQuery('SELECT * FROM college_periods WHERE PERIOD_ID=' . $cp_per_id['PERIOD_ID']));
                    $check_sp = $check_sp[1];
                ...
                $check_take_attn = DBGet(DBQuery('SELECT COUNT(*) AS TOTAL FROM attendance_period WHERE COURSE_PERIOD_ID=' . $_REQUEST['course_period_id'] . ' AND college_date=\'' . $_REQUEST['meet_date'] . '\''));
                $check_miss_attn = DBGet(DBQuery('SELECT COUNT(*) AS TOTAL FROM missing_attendance WHERE COURSE_PERIOD_ID=' . $_REQUEST['course_period_id'] . ' AND college_date=\'' . $_REQUEST['meet_date'] . '\''));
        ...
        DBQuery("UPDATE course_period_var cpv,college_periods sp SET cpv.start_time=sp.start_time,cpv.end_time=sp.end_time WHERE sp.period_id=cpv.period_id AND course_period_id=$_REQUEST[course_period_id]");
...
if (!$_REQUEST['modfunc'] && !$_REQUEST['course_modfunc'] && !$_REQUEST['action']) {
    ...
    if (clean_param($_REQUEST['course_period_id'], PARAM_ALPHANUM)) {
        ...
        $cal_RET = DBGet(DBQuery("SELECT TITLE,CALENDAR_ID FROM college_calendars WHERE SYEAR='" . UserSyear() . "' AND COLLEGE_ID='" . UserCollege() . "' ORDER BY DEFAULT_CALENDAR DESC"));
        $options = array();
        $header .= '<div class="col-sm-6 col-lg-4">';
        ...
        if ($_REQUEST[course_period_id] == 'new')
            ...
            $cal_sql = "SELECT TITLE,CALENDAR_ID FROM college_calendars WHERE SYEAR='" . UserSyear() . "' AND COLLEGE_ID='" . UserCollege() . "' AND CALENDAR_ID='" . $RET['CALENDAR_ID'] . "'";
        ...
        $header .= '</div>'; //.col-sm-6.col-lg-4
          //echo "SELECT STAFF_ID,LAST_NAME,FIRST_NAME,MIDDLE_NAME FROM staff INNER JOIN staff_college_relationship USING (staff_id) WHERE college_id='" . UserCollege() . "' AND SYEAR='" . UserSyear() . "' AND PROFILE='teacher' AND  (ISNULL(IS_DISABLE) OR IS_DISABLE!='Y') AND (END_DATE>=CURDATE() OR END_DATE IS NULL OR END_DATE='0000-00-00') AND (START_DATE<=CURDATE() OR START_DATE='0000-00-00' OR START_DATE IS NULL ) ORDER BY LAST_NAME,FIRST_NAME ";
        //$teachers_RET = DBGet(DBQuery("SELECT STAFF_ID,LAST_NAME,FIRST_NAME,MIDDLE_NAME FROM staff INNER JOIN staff_college_relationship USING (staff_id) WHERE college_id='" . UserCollege() . "' AND SYEAR='" . UserSyear() . "' AND PROFILE='teacher' AND  (ISNULL(IS_DISABLE) OR IS_DISABLE!='Y') AND (END_DATE>=CURDATE() OR END_DATE IS NULL OR END_DATE='0000-00-00') AND (START_DATE<=CURDATE() OR START_DATE='0000-00-00' OR START_DATE IS NULL ) ORDER BY LAST_NAME,FIRST_NAME "));
        
      $teachers_RET= DBGet(DBQuery("SELECT STAFF_ID,LAST_NAME,FIRST_NAME,MIDDLE_NAME FROM staff s INNER JOIN staff_college_relationship ssr USING(staff_id) ,login_authentication la WHERE (s.PROFILE_ID!=4 OR s.PROFILE_ID IS NULL) AND ssr.SYEAR='".UserSyear()."' AND s.STAFF_ID=la.USER_ID AND la.PROFILE_ID NOT IN (3,4) AND COLLEGE_ID=".UserCollege()." AND (s.IS_DISABLE<>'Y' OR s.IS_DISABLE IS NULL) AND (ssr.END_DATE>='CURDATE()' OR ssr.END_DATE='0000-00-00' OR ssr.END_DATE IS NULL) AND PROFILE='teacher' ORDER BY LAST_NAME,FIRST_NAME")) ;
        ...
        if ($_REQUEST['course_period_id'] != 'new' && $RET['AVAILABLE_SEATS'] != $RET['TOTAL_SEATS'])
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">Primary Teacher</label><div class="col-md-8">' . SelectInputDisabledMsg($RET['TEACHER_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][TEACHER_ID]', '', $teachers, 'N/A', '', $div, "To Change teacher go to College Setup->Courses->Teacher Re-Assignment") . '</div></div>';
        else
...
//        echo $RET['DOES_BREAKOFF'];
        ...
        unset($options);
        $mp_RET = DBGet(DBQuery("SELECT MARKING_PERIOD_ID,SHORT_NAME,'2' AS t,SORT_ORDER FROM college_quarters WHERE COLLEGE_ID='" . UserCollege() . "' AND SYEAR='" . UserSyear() . "' UNION SELECT MARKING_PERIOD_ID,SHORT_NAME,'1' AS t,SORT_ORDER FROM college_semesters WHERE COLLEGE_ID='" . UserCollege() . "' AND SYEAR='" . UserSyear() . "' UNION SELECT MARKING_PERIOD_ID,SHORT_NAME,'0' AS t,SORT_ORDER FROM college_years WHERE COLLEGE_ID='" . UserCollege() . "' AND SYEAR='" . UserSyear() . "' ORDER BY 3,4"));
        ...
        if ($_REQUEST['course_period_id'] != 'new' || $not_pass == true) {
            ...
            $days_RET = DBGet(DBQuery("SELECT DAYS FROM college_calendars WHERE calendar_id=$RET[CALENDAR_ID]"));
        ...
        if ($_REQUEST['course_period_id'] == 'new' && $not_pass == false) {
            ...
            $calendar_RET = DBGet(DBQuery('SELECT DATE_FORMAT(COLLEGE_DATE,\'%d-%b-%y\') as COLLEGE_DATE,MINUTES,BLOCK FROM attendance_calendar WHERE  SYEAR=\'' . UserSyear() . '\' AND COLLEGE_ID=\'' . UserCollege() . '\' AND CALENDAR_ID=\'' . $RET['CALENDAR_ID'] . '\''), array(), array('COLLEGE_DATE'));
        ...
        $grade_level_RET = DBGet(DBQuery("SELECT ID,TITLE FROM college_gradelevels WHERE college_id='" . UserCollege() . "'"));
        if ($_REQUEST['course_id'] != 'new') {
            $sql = "SELECT c.TITLE,sg.TITLE AS GRADE_LEVEL_TITLE,c.SHORT_NAME,GRADE_LEVEL
						FROM courses c LEFT JOIN college_gradelevels sg ON c.grade_level=sg.id
						WHERE COURSE_ID='$_REQUEST[course_id]'";
            $QI = DBQuery($sql);
            $RET = DBGet($QI);
            $RET = $RET[1];
