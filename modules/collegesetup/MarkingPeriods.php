...
{ 
	...
	// ---------------------- Insert & Update Start ------------------------------ //
		foreach($_REQUEST['tables'] as $id=>$columns)
		{
                    ...
                    if($table=='college_progress_periods')
                    {
                                                                                    ...
                                                                                    else
                                                                                    {
                                                                                        ...
                                                                                        else
                                                                                        {
                                                                                            ...
                                                                                            else
                                                                                            {
                                                                                                ...
                                                                                                {
                                                                                                $err_msg="Start date cannot be changed because the FULL YEAR with college periods";
                                                                                                    break 2; 
                                                                                                }
                                                                                                else
                                                                                                {
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='delete')
{
	...
	switch($table)
	{
		case 'college_years':
			...
			$extra[] = 'DELETE FROM college_progress_periods WHERE QUARTER_ID IN (SELECT MARKING_PERIOD_ID FROM college_quarters WHERE SEMESTER_ID IN (SELECT MARKING_PERIOD_ID FROM college_semesters WHERE YEAR_ID=\''.$year_id.'\'))';
			$extra[] = 'DELETE FROM college_quarters WHERE SEMESTER_ID IN (SELECT MARKING_PERIOD_ID FROM college_semesters WHERE YEAR_ID=\''.$year_id.'\')';
			$extra[] = 'DELETE FROM college_semesters WHERE YEAR_ID=\''.$year_id.'\'';
		break;

		case 'college_semesters':
			$name = 'semester';
			$parent_term = 'FY'; $parent_id = paramlib_validation($column=MARKING_PERIOD_ID,$_REQUEST['year_id']);
                                                      $parent_table='college_years';
            $sems_id=paramlib_validation($column=MARKING_PERIOD_ID,$_REQUEST[marking_period_id]);
			$extra[] = 'DELETE FROM college_progress_periods WHERE QUARTER_ID IN (SELECT MARKING_PERIOD_ID FROM college_quarters WHERE SEMESTER_ID=\''.$sems_id.'\')';
			$extra[] = 'DELETE FROM college_quarters WHERE SEMESTER_ID=\''.$sems_id.'\'';
		break;

		case 'college_quarters':
			$name = 'quarter';
			$parent_term = 'SEM'; $parent_id = paramlib_validation($column=MARKING_PERIOD_ID,$_REQUEST['semester_id']);
                                                      $parent_table='college_semesters';
            $qrt_id=paramlib_validation($column=MARKING_PERIOD_ID,$_REQUEST[marking_period_id]);
			$extra[] = 'DELETE FROM college_progress_periods WHERE QUARTER_ID=\''.$qrt_id.'\'';
		break;
...
$has_assigned_RET=DBGet(DBQuery('SELECT COUNT(*) AS TOTAL_ASSIGNED FROM course_details WHERE MARKING_PERIOD_ID=\''.paramlib_validation($column=MARKING_PERIOD_ID,$_REQUEST[marking_period_id]).'\' OR MARKING_PERIOD_ID IN(SELECT MARKING_PERIOD_ID FROM marking_periods WHERE PARENT_ID=\''.paramlib_validation($column=MARKING_PERIOD_ID,$_REQUEST[marking_period_id]).'\')'));
	...
	if($has_assigned>0){
        ...
        if($table=='college_quarters')
        $check_other_assocs=DBGet(DBQuery('SELECT COUNT(1) as REC_EX FROM college_progress_periods WHERE QUARTER_ID='.$_REQUEST['marking_period_id']));
        else
...
//        }
        ...
        echo '<div>';
        if (clean_param($_REQUEST['marking_period_id'], PARAM_ALPHANUM) == 'new'){
           echo AllowEdit() ? '<hr/><div class="text-right"><INPUT type=submit value=Save class="btn btn-primary" onclick="return formcheck_college_setup_marking();"></div>' : '';
    ...
    if (($_REQUEST['mp_term'] == 'FY' && $_REQUEST['marking_period_id'] != 'new') || $_REQUEST['mp_term'] == 'SEM' || $_REQUEST['mp_term'] == 'QTR' || $_REQUEST['mp_term'] == 'PRO') {
        ...
        if (($_REQUEST['mp_term'] == 'SEM' && $_REQUEST['marking_period_id'] != 'new') || $_REQUEST['mp_term'] == 'QTR' || $_REQUEST['mp_term'] == 'PRO') {
         ...
         /*   echo '<div class="col-md-4">';
            ...
            $sem_edate = DBGet(DBQuery('SELECT END_DATE,START_DATE FROM college_semesters WHERE MARKING_PERIOD_ID=' . $_REQUEST[semester_id]));
            $sem_sdate = $sem_edate[1]['START_DATE'];
            $sem_edate = $sem_edate[1]['END_DATE'];
            $qtr_edate = DBGet(DBQuery('SELECT MAX(END_DATE) AS END_DATE,MIN(START_DATE) as START_DATE FROM college_quarters WHERE SEMESTER_ID=' . $_REQUEST[semester_id]));
            ...
            if (($_REQUEST['mp_term'] == 'QTR' && $_REQUEST['marking_period_id'] != 'new') || $_REQUEST['mp_term'] == 'PRO') {
                ...
                $sql_mp_id = 'SELECT MARKING_PERIOD_ID,TITLE FROM college_progress_periods';
            }
        }
    }
