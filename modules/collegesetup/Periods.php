    ...
    foreach ($_REQUEST['values'] as $id => $columns) {
        if (!(isset($columns['TITLE']) && trim($columns['TITLE']) == '')) {
            ...
            if ($id != 'new' && $columns['START_TIME'] != '' && !isset($columns['END_TIME'])) {
                $period_etime = DBGet(DBQuery('SELECT END_TIME FROM  college_periods WHERE period_id=\'' . $id . '\''));
            ...
            if ($id != 'new' && $columns['END_TIME'] != '' && !isset($columns['START_TIME'])) {
                $period_etime = DBGet(DBQuery('SELECT START_TIME FROM  college_periods WHERE period_id=\'' . $id . '\''));
            ...
            //  print_r($columns);
            //  echo '</pre>';
                $sql = 'UPDATE college_periods SET ';
                $title_change = '';
                foreach ($columns as $column => $value) {
                    ...
                    if ($column == 'ignore_scheduling' && $value == '') {
                        ...
                        if ($checker[1]['TOTAL'] == 0) {
                            ...
                            $check_for_change=DBGet(DBQuery('SELECT COUNT(*) AS REC_EX FROM college_periods WHERE PERIOD_ID='.$id.' AND '.$column.'=\''.$value.'\'' ));
                        ...
                        if ($column == 'TITLE' && !isset($_REQUEST['values'][$id]['SHORT_NAME'])) {
                            $exist_pr_title = DBGet(DBQuery('SELECT count(PERIOD_ID) AS TOT FROM  college_periods WHERE title=\'' . $value . '\' AND SHORT_NAME=\'' . $exist_pr[1]['SHORT_NAME'] . '\' and period_id<>\'' . $id . '\''));
                        ...
                        if ($column == 'SHORT_NAME' && !isset($_REQUEST['values'][$id]['TITLE'])) {
                            $exist_pr_title = DBGet(DBQuery('SELECT count(PERIOD_ID) AS TOT FROM  college_periods WHERE title=\'' . $exist_pr[1]['TITLE'] . '\' AND SHORT_NAME=\'' . $value . '\' and period_id<>\'' . $id . '\''));
                ...
                $sql_get_length = 'SELECT start_time, end_time from college_periods WHERE period_id=\'' . $id . '\'';
                ...
                $sql_length_update = 'UPDATE college_periods set length = ' . $length . ' where period_id=\'' . $id . '\'';
...
//                    } else {
                        ...
                        $sql_up = 'update college_periods set length = ' . $length . ' where period_id=\'' . $period_id . '\' and syear=\'' . UserSyear() . '\' and college_id=\'' . UserCollege() . '\'';
...
DrawBC("College Setup <i class=\"icon-arrow-right13\"></i> " . ProgramTitle());
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'remove' && AllowEdit()) {
    ...
    if ($has_assigned > 0) {
        ...
        if (DeletePrompt_Period('period')) {
            DBQuery('DELETE FROM college_periods WHERE PERIOD_ID=\'' . $prd_id . '\'');
...
if ($_REQUEST['modfunc'] != 'remove') {
    ...
    echo '<hr class="no-margin"/><div class="panel-body text-right">' . SubmitButton('Save', '', 'class="btn btn-primary" onclick="formcheck_college_setup_periods();"') . '</div>';
    echo '</div>';
    echo '</FORM>';
}
...
function _makeTimeInput($value, $name) {
    ...
    if ($id != 'new') {
        $sql_ampm_s = 'SELECT START_TIME FROM college_periods WHERE period_id=' . $id;
...
function _makeTimeInputEnd($value, $name) {
    ...
    if ($id != 'new') {
        $sql_ampm = 'select end_time from college_periods where period_id=' . $id;
