...
PopTable('header', 'Upload College Logo');

//$CollegeLogoPath = 'assets/collegelogo';
//if (!file_exists($CollegeLogoPath)) {
//    mkdir($CollegeLogoPath);
//}

if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'edit') {

//    if ($CollegeLogoPath && (($file = @fopen($picture_path = $_SESSION['logo_path'], 'r')) )) {
...
//        $target_path = $CollegeLogoPath . '/' . substr($_FILES["file"]["name"], 0, strrpos($_FILES["file"]["name"], ".")) . UserCollege() . '.' . $upload->get_file_extension($upload->name);
//
//        $destination_path = $CollegeLogoPath;
...
//            echo '
//	' . $msg . '
//	<form enctype="multipart/form-data" action="Modules.php?modname=collegesetup/UploadLogo.php&action=upload" method="POST">';
//            echo '<div align=center>Select Logo: <input name="file" type="file" /><br /><br>
//<input type="submit" value="Upload" name="Submit" class="btn btn-primary" />&nbsp;<input type=button class="btn btn-default" value=Cancel onclick=\'load_link(Modules.php?modname=collegesetup/Colleges.php);\'></div>
//</form>';
//            PopTable('footer');
//        } 
...
<input type="submit" value="Upload" name="Submit" class=btn_medium />&nbsp;<input type=button class=btn_medium value=Cancel onclick=\'load_link("Modules.php?modname=collegesetup/Colleges.php");\'></div>
</form>';
            PopTable('footer');
...
//            echo '<div align=center><input type=button class="btn btn-primary" value=Done onclick=\'load_link(Modules.php?modname=collegesetup/Colleges.php);\'></div>';
//            PopTable('footer');
...
//fclose($upload->target_path);
//      $filename =  $upload->target_path;
            echo '<div align=center><input type=button class=btn_medium value=Done onclick=\'load_link("Modules.php?modname=collegesetup/Colleges.php");\'></div>';
            PopTable('footer');
        }
    } else {
        echo '
...
<form enctype="multipart/form-data" action="Modules.php?modname=collegesetup/UploadLogo.php&action=upload" method="POST">';
        echo '<div align=center>Select Logo: <input name="file" type="file" /><b><span >(Maximum upload file size 10 MB)</span></b><br /><br>
<input type="submit" name="Submit"  value="Upload" class="btn btn-primary" />&nbsp;<input type=button class="btn btn-default" value=Cancel onclick=\'load_link("Modules.php?modname=collegesetup/Colleges.php");\'></div>
</form>';
        PopTable('footer');
    }
} else {
    echo 'Please select a college first!';
    PopTable('footer');
}
?>