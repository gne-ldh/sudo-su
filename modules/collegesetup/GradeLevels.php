...
if(clean_param($_REQUEST['values'],PARAM_NOTAGS) && ($_POST['values'] || $_REQUEST['ajax']))
{
    foreach($_REQUEST['values'] as $id=>$columns)
    {
            ...
            { $sql1='SELECT TITLE,SHORT_NAME,SORT_ORDER FROM college_gradelevels WHERE COLLEGE_ID =\''.UserCollege().'\' AND ID!='.$id;
                ...
                $sql = 'UPDATE college_gradelevels SET ';
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='remove')
{
	...
	if(DeletePrompt_GradeLevel('grade level'))
	{
		...
		DBQuery("DELETE FROM college_gradelevels WHERE ID='$grd_id'");
                DBQuery('UPDATE college_gradelevels SET NEXT_GRADE_ID=NUll WHERE NEXT_GRADE_ID='.$grd_id);
...
$grade_id=implode(',',$grade_id_arr);
	...
	if(AllowEdit()){
            echo '<hr class="no-margin"/><div class="panel-body text-right"><INPUT class="btn btn-primary" type=submit value=Save onclick="formcheck_college_setup_grade_levels();"></div>';
        }
        echo '</div>';
	echo '</FORM>';
}
