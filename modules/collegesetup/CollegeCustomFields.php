...
if (clean_param($_REQUEST['tables'], PARAM_NOTAGS) && ($_POST['tables'] || $_REQUEST['ajax'])) {
    ...
    foreach ($_REQUEST['tables'] as $id => $columns) {
        ...
        if ($id != 'new') {
            ...
            $go = true;
            if ($table == 'college_custom_fields')
                $custom_field_id = $id;

            // print_r($_REQUEST);exit;
            if ($custom_field_id) {

                $chk_cus_data = DBGet(DBQuery('SELECT * from colleges WHERE SYEAR=' . UserSyear() . ' AND id=' . UserCollege()));
                if ($chk_cus_data[1]['CUSTOM_' . $custom_field_id] != '') {
                    $flag = 1;
                }
                $custom_update = DBGet(DBQuery("SELECT TYPE,REQUIRED,DEFAULT_SELECTION,HIDE FROM college_custom_fields WHERE ID=$custom_field_id"));
                $custom_update = $custom_update[1];
                switch ($custom_update['TYPE']) {
                    case 'radio':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id VARCHAR(1) ";
                        break;

                    case 'text':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id VARCHAR(255)";
                        break;

                    case 'select':
                    case 'autos':
                    case 'edits':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id VARCHAR(100)";
                        break;

                    case 'codeds':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id VARCHAR(15)";
                        break;
                    ...
                    case 'numeric':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id NUMERIC(20,2)";
                    ...
                    case 'date':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id  DATE";
                    ...
                    case 'textarea':
                        $Sql_modify_column = "ALTER TABLE colleges MODIFY CUSTOM_$id LONGTEXT";
                        $not_default = true;
                        break;
                }
                ...
                if ($flag == 0) {
                    ...
                    if($sort_oder_to_change!=0){
                        DBQuery('UPDATE college_custom_fields SET SORT_ORDER=\''.$sort_oder_to_change.'\' WHERE ID='.$custom_field_id);
                    }
                    else{
            ...
            if ($table == 'college_custom_fields') {
                ...
                $id = DBGet(DBQuery("SHOW TABLE STATUS LIKE 'college_custom_fields'"));
                ...
                switch ($columns['TYPE']) {
                    case 'radio':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id VARCHAR(1) ";
                        break;

                    case 'text':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id VARCHAR(255)";
                        break;

                    case 'select':
                    case 'autos':
                    case 'edits':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id VARCHAR(100)";
                        break;

                    case 'codeds':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id VARCHAR(15)";
                        break;
                    ...
                    case 'numeric':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id NUMERIC(20,2)";
                        if (!is_numeric($columns['DEFAULT_SELECTION'])) {
                            $not_default = true;
                    ...
                    case 'date':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id  DATE";
                        if (preg_match("/^[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}$/", $columns['DEFAULT_SELECTION']) === 0) {
                            $not_default = true;
                            $columns['DEFAULT_SELECTION'] = '';
                        }
                    ...
                    case 'textarea':
                        $Sql_add_column = "ALTER TABLE colleges ADD CUSTOM_$id LONGTEXT";
                        $not_default = true;
                        break;
                }
                if ($columns['REQUIRED']) {
...
//echo $sql;
    ...
    echo '<script>window.location.href="Modules.php?modname=collegesetup/CollegeCustomFields.php"</script>';
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'delete') {
    if (clean_param($_REQUEST['id'], PARAM_INT)) {
        ...
        $chk_sch_cus = DBGet(DBQuery('SELECT COUNT(*) AS REC_EX FROM colleges WHERE (' . $sc_cus_fld_column . '<>\'\' AND  ' . $sc_cus_fld_column . ' is NOT NULL)'));

        if ($chk_sch_cus[1]['REC_EX'] == 0) {
            if (DeletePromptCommon('college field')) {
                $id = clean_param($_REQUEST['id'], PARAM_INT);
                DBQuery('DELETE FROM college_custom_fields WHERE ID=\'' . $id . '\'');
                DBQuery('ALTER TABLE colleges DROP COLUMN CUSTOM_' . $id . '');
                $_REQUEST['modfunc'] = '';
                unset($_REQUEST['id']);
            }
        } else {
...
if ($_REQUEST['id'] && !$_REQUEST['modfunc']) {
    ...
    if ($_REQUEST['id'] != 'new')
        ...
        $action .= "&table=college_custom_fields";    
...
//    echo $system_wide;
    ...
    $header .= '</div>';
    $header .= '<div class="panel-footer"><div class="heading-elements">' . SubmitButton('Save', '', 'class="btn btn-primary heading-btn pull-right ml-10" onclick="formcheck_collegefields();"') . $delete_button . '</div></div>';
    $header .= '</div>'; //.panel
    ...
    ListOutput($dd, $columns, 'College Field', 'College Fields', $link, array(), $LO_options);
    echo '</div>'; //.panel
