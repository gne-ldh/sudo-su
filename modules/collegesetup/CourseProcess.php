...
$_REQUEST['modname'] = 'collegesetup/Courses.php';
...
switch ($task) {
    case 'md':
        ...
        if ($calendar_id != '') {
            $days_RET = DBGet(DBQuery("SELECT DAYS FROM college_calendars WHERE calendar_id=$calendar_id"));
        ...
        if ($_REQUEST['sch_type'] == 'variable') {
            ...
            if ($_REQUEST['msg'] != 'conflict')
                ...
                $cpdays_RET = DBGet(DBQuery("SELECT START_TIME,END_TIME FROM college_periods where period_id=$_REQUEST[period_id]"));
