...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'choose_course') {
    ...
    if (count($RET)) {
        ...
        $sql = "SELECT TITLE,START_TIME,END_TIME FROM college_periods WHERE PERIOD_ID='" . $RET['PERIOD_ID'] . "'";
        ...
        if ($RET['CALENDAR_ID'] != '') {
            $sql = "SELECT TITLE,CALENDAR_ID FROM college_calendars WHERE CALENDAR_ID='" . $RET['CALENDAR_ID'] . "'";
    ...
    if ($_REQUEST['print'] != 'list') {
        ...
        echo '<div class="panel-heading">
                <h6 class="panel-title">Course Catalog</h6>
...
function CreateList($dli = '', $pli = '', $sli = '', $cli = '', $mp = '', $mp_name = '') {
    ...
    if ($mp == '') {
        ...
        if ($sli == '') {
            ...
            if ($cli == '') {
                $where = 'and marking_period_id=\'' . $mp . '\' and course_id in (select Course_Id from courses where subject_id = \'' . $_REQUEST['subject_id'] . '\' and College_Id=\'' . UserCollege() . '\')';
    ...
    $sql = 'select
                ...
                short_name,(select CONCAT(START_TIME,\' - \',END_TIME,\' \') from college_periods where period_id=course_period_var.period_id) as period_time, (select title from college_periods where period_id=course_period_var.period_id) as period, marking_period_id, (select title from marking_periods where marking_period_id=course_periods.marking_period_id) as mp,
                (select CONCAT(LAST_NAME,\' \',FIRST_NAME,\' \') from staff where staff_id=course_periods.teacher_id) as teacher, rooms.title as location,days,course_periods.course_period_id,course_periods.begin_date,course_periods.end_date,course_periods.SCHEDULE_TYPE,course_period_var.ID AS CPV_ID from course_periods,course_period_var,rooms where course_periods.college_id=\'' . UserCollege() . '\' and course_period_var.room_id=rooms.room_id and course_periods.course_period_id=course_period_var.course_period_id and course_periods.syear=\'' . UserSyear() . '\' ' . $where . '  GROUP BY course_period_var.COURSE_PERIOD_ID ORDER BY course_period_var.ID';
    ...
    foreach ($ret_temp as $ri => $rd) {
        ...
        if ($rd['SCHEDULE_TYPE'] == 'FIXED')
            ...
            $get_det = DBGet(DBQuery('SELECT cpv.*,CONCAT(sp.START_TIME,\' - \',sp.END_TIME,\' \') as PERIOD_TIME,sp.TITLE as PERIOD,r.TITLE AS LOCATION FROM course_period_var cpv,college_periods sp,rooms r WHERE cpv.COURSE_PERIOD_ID=' . $rd['COURSE_PERIOD_ID'] . ' AND cpv.PERIOD_ID=sp.PERIOD_ID AND cpv.ROOM_ID=r.ROOM_ID'));
            $ret[$i] = $rd;
            if (count($get_det) > 1) {
    ...
    $html .= "<tr><td  style=\"font-size:15px; font-weight:bold;\">" . GetCollege(UserCollege()) . "<div style=\"font-size:12px;\">Student Schedules Report</div></td><td align=right style=\"padding-top:10px;\">" . ProperDate(DBDate()) . "<br />Powered by openSIS</td></tr><tr><td colspan=2 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
    ...
    if ($_REQUEST['print'] == 'list') {
        ...
        echo "<tr><td  style=\"font-size:15px; font-weight:bold;\">" . GetCollege(UserCollege()) . "<div style=\"font-size:12px;\">Course Catalog</div></td><td align=right style=\"padding-top:5px;\">" . ProperDate(DBDate()) . "<br />Powered by openSIS</td></tr><tr><td colspan=2 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
    }
...
function CreateExcel($dli = '', $pli = '', $sli = '', $cli = '', $mp = '', $mp_name = '') {
    ...
    $sql = 'select
                ...
                (select title from course_subjects where subject_id=(select subject_id from courses where 						course_id=course_periods.course_id)) as subject,
                short_name,
                (select title from college_periods where period_id=cpv.period_id) as period,
                marking_period_id,
                ...
                from course_periods,cours_period_var cpv,rooms r where course_period.college_id=\'' . UserCollege() . '\' and course_period.course_period_id=cpv.course_period_id and r.room_id=cpv.room_id and course_period.syear=\'' . UserSyear() . '\' ' . $where . '';
