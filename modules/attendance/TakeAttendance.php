...
$sc_years_sql = 'SELECT MARKING_PERIOD_ID FROM college_years WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND \'' . date('Y-m-d', strtotime($date)) . '\' BETWEEN START_DATE AND END_DATE';
$sc_years_data = DBGet(DBQuery($sc_years_sql));
$sc_years_data = $sc_years_data[1]['MARKING_PERIOD_ID'];

$sc_sem_sql = 'SELECT MARKING_PERIOD_ID FROM college_semesters WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND \'' . date('Y-m-d', strtotime($date)) . '\' BETWEEN START_DATE AND 
END_DATE';
$sc_sem_data = DBGet(DBQuery($sc_sem_sql));
$sc_sem_data = $sc_sem_data[1]['MARKING_PERIOD_ID'];

$sc_qtr_sql = 'SELECT MARKING_PERIOD_ID FROM college_quarters WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND \'' . date('Y-m-d', strtotime($date)) . '\' BETWEEN START_DATE AND 
END_DATE';
...
FROM attendance_calendar acc,course_periods cp,course_period_var cpv,college_periods sp 
WHERE acc.SYEAR=\'' . UserSyear() . '\' 
AND cp.COLLEGE_ID=acc.COLLEGE_ID 
...
AND ((sp.BLOCK IS NULL AND position(substring(\'UMTWHFS\' FROM DAYOFWEEK(acc.COLLEGE_DATE) FOR 1) IN cpv.DAYS)>0
OR sp.BLOCK IS NOT NULL 
AND acc.BLOCK IS NOT NULL 
AND sp.BLOCK=acc.BLOCK) or cpv.course_period_date=acc.college_date)
...
$current_Q = 'SELECT ATTENDANCE_TEACHER_CODE,COLLEGE_ROLL_NO,ADMIN,COMMENT FROM ' . $table . ' WHERE COLLEGE_DATE=\'' . date('Y-m-d', strtotime($date)) . '\'AND PERIOD_ID=\'' . UserPeriod() . '\' AND COURSE_PERIOD_ID=\'' . $cq_cpid . '\'' . ($table == 'lunch_period' ? ' AND TABLE_NAME=\'' . $tabl . '\'' : '');
...
if ($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax'])) {
    ...
    if ($_REQUEST['table'] == '0') {
        ...
        $RET = DBGet(DBQuery('SELECT \'completed\' AS COMPLETED FROM attendance_completed WHERE (STAFF_ID=\'' . User('STAFF_ID') . '\' OR SUBSTITUTE_STAFF_ID=\'' . User('STAFF_ID') . '\') AND COLLEGE_DATE=\'' . date('Y-m-d', strtotime($date)) . '\' AND PERIOD_ID=\'' . UserPeriod() . '\' and course_period_id=\'' . ($cq_cpid != '' ? $cq_cpid : UserCoursePeriod()) . '\''));
        if (!count($RET)) {
            ...
            if ($secondary_teacher_id == User('STAFF_ID'))
                DBQuery('INSERT INTO attendance_completed (STAFF_ID,COLLEGE_DATE,PERIOD_ID,COURSE_PERIOD_ID,CPV_ID,SUBSTITUTE_STAFF_ID,IS_TAKEN_BY_SUBSTITUTE_STAFF) values(\'' . $teacher_type[1]['TEACHER_ID'] . '\',\'' . $date . '\',\'' . UserPeriod() . '\',\'' . ($cq_cpid != '' ? $cq_cpid : UserCoursePeriod()) . '\',\'' . CpvId() . '\',\'' . $secondary_teacher_id . '\',\'Y\')');
            elseif ($teacher_id == User('STAFF_ID'))
                DBQuery('INSERT INTO attendance_completed (STAFF_ID,COLLEGE_DATE,PERIOD_ID,COURSE_PERIOD_ID,CPV_ID,SUBSTITUTE_STAFF_ID) values(\'' . $teacher_type[1]['TEACHER_ID'] . '\',\'' . $date . '\',\'' . UserPeriod() . '\',\'' . ($cq_cpid != '' ? $cq_cpid : UserCoursePeriod()) . '\',\'' . CpvId() . '\',\'' . $secondary_teacher_id . '\')');
            else
                DBQuery('INSERT INTO attendance_completed (STAFF_ID,COLLEGE_DATE,PERIOD_ID,COURSE_PERIOD_ID,CPV_ID,SUBSTITUTE_STAFF_ID) values(\'' . User('STAFF_ID') . '\',\'' . $date . '\',\'' . UserPeriod() . '\',\'' . ($cq_cpid != '' ? $cq_cpid : UserCoursePeriod()) . '\',\'' . CpvId() . '\',\'' . $secondary_teacher_id . '\')');
        }
        DBQuery('DELETE FROM missing_attendance WHERE  COLLEGE_DATE=\'' . $date . '\' AND COURSE_PERIOD_ID=\'' . ($cq_cpid != '' ? $cq_cpid : UserCoursePeriod()) . '\'  AND(TEACHER_ID=\'' . User('STAFF_ID') . '\' OR SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\')  AND PERIOD_ID=\'' . UserPeriod() . '\'');
    }
...
if ($_SESSION['PROFILE'] == 'teacher')
    $codes_RET_count = DBGet(DBQuery('SELECT COUNT(*) AS CODES FROM attendance_codes WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND SYEAR=\'' . UserSyear() . '\'  AND TYPE=\'teacher\' AND TABLE_NAME=\'' . $_REQUEST['table'] . '\'' . ($course_RET[1]['HALF_DAY'] ? ' AND STATE_CODE!=\'H\'' : '') . ' ORDER BY SORT_ORDER'));

$codes_RET = DBGet(DBQuery('SELECT ID,TITLE,DEFAULT_CODE,STATE_CODE FROM attendance_codes WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND SYEAR=\'' . UserSyear() . '\'  AND TABLE_NAME=\'' . $_REQUEST['table'] . '\'' . ($course_RET[1]['HALF_DAY'] ? ' AND STATE_CODE!=\'H\'' : '') . ' ORDER BY SORT_ORDER'));
if (count($codes_RET)) {
    ...
    $columns = array();
$extra['SELECT'] .= ',s.COLLEGE_ROLL_NO AS COMMENT';
...
if ($_REQUEST['table'] == '0') {
    $completed_RET = DBGet(DBQuery('SELECT \'Y\' as COMPLETED,STAFF_ID,SUBSTITUTE_STAFF_ID,IS_TAKEN_BY_SUBSTITUTE_STAFF FROM attendance_completed WHERE (STAFF_ID=\'' . User('STAFF_ID') . '\' OR SUBSTITUTE_STAFF_ID=\'' . User('STAFF_ID') . '\') AND COLLEGE_DATE=\'' . $date . '\' AND PERIOD_ID=\'' . UserPeriod() . '\' AND CPV_ID=\'' . CpvId() . '\''));
    if ($completed_RET) {
    ...
    if ($_SESSION['miss_attn'] == 1) {
        ...
        if (($_REQUEST['username'] == 'admin' || $get_profile_type[1]['PROFILE'] == 'admin'))  //&& $_REQUEST['modfunc']=='attn'
            $note1 = '<a href=Modules.php?modname=users/TeacherPrograms.php?include=attendance/MissingAttendance.php&From=' . $From . '&to=' . $to . ' class="text-primary"><i class="icon-square-left"></i> Back to Missing Attendance List </a>';
    }
}
...
if ($profile[1]['PROFILE'] != "admin" && UserCoursePeriod() != '') {
    if (isset($_REQUEST['cp_id_miss_attn'])) {
        $QI = DBQuery('SELECT DISTINCT cpv.ID,cpv.PERIOD_ID,cp.COURSE_PERIOD_ID,sp.TITLE,sp.SHORT_NAME,cp.MARKING_PERIOD_ID,cpv.DAYS,sp.SORT_ORDER,c.TITLE AS COURSE_TITLE FROM course_periods cp,course_period_var cpv, college_periods sp,courses c WHERE c.COURSE_ID=cp.COURSE_ID AND cpv.PERIOD_ID=sp.PERIOD_ID AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cpv.COURSE_PERIOD_ID=' . $_REQUEST['cp_id_miss_attn'] . ' AND cp.SYEAR=\'' . UserSyear() . '\' AND cp.COLLEGE_ID=\'' . UserCollege() . '\' AND (cp.TEACHER_ID=\'' . User('STAFF_ID') . '\' OR cp.SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\') ORDER BY sp.SORT_ORDER ');
        $RET = DBGet($QI);
        $period_select = '<label class="control-label">Choose Period:</label>
        ...
        $QI = DBQuery('SELECT DISTINCT cpv.ID,cpv.PERIOD_ID,cp.COURSE_PERIOD_ID,sp.TITLE,sp.SHORT_NAME,cp.MARKING_PERIOD_ID,cpv.DAYS,sp.SORT_ORDER,c.TITLE AS COURSE_TITLE FROM course_periods cp,course_period_var cpv, college_periods sp,courses c WHERE c.COURSE_ID=cp.COURSE_ID AND cpv.PERIOD_ID=sp.PERIOD_ID AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cpv.COURSE_PERIOD_ID=' . UserCoursePeriod() . ' AND cp.SYEAR=\'' . UserSyear() . '\' AND cp.COLLEGE_ID=\'' . UserCollege() . '\' AND (cp.TEACHER_ID=\'' . User('STAFF_ID') . '\' OR cp.SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\') ORDER BY sp.SORT_ORDER ');
        $RET = DBGet($QI);
...
if (!$mp_id) {
    echo '<div class="panel-body p-t-0 p-b-0"><div class="alert alert-danger alert-bordered">The selected date is not in a college quarter.</div></div>';
} else {
    if (count($course_RET) != 0) {
        ...
        if ($_REQUEST['period']) {
            $extra_sql = 'SELECT COUNT(*) AS day_exist FROM attendance_calendar WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND SYEAR=\'' . UserSyear() . '\' AND COLLEGE_DATE=\'' . date('Y-m-d', strtotime($date)) . '\'';
            $extra_ret = DBGET(DBQuery($extra_sql));
            if (isset($extra_ret[1]['DAY_EXIST']) && $extra_ret[1]['DAY_EXIST'] == 0)
                echo '<div class="panel-body p-t-0 p-b-0"><div class="alert alert-danger alert-bordered">You cannot take attendance on this day, it is a college holiday</div>';
            else
...
function _makeRadio($value, $title) {
    ...
    $flag = false;
    if ($current_RET[$THIS_RET['COLLEGE_ROLL_NO']][1]['ATTENDANCE_TEACHER_CODE']) {
        $attn_code_type = DBGet(DBQuery('SELECT TYPE FROM attendance_codes WHERE ID=' . $current_RET[$THIS_RET['COLLEGE_ROLL_NO']][1]['ATTENDANCE_TEACHER_CODE']));
        if ($attn_code_type[1]['TYPE'] == 'official')
            $flag = true;
    }
    $colors = array('P' => 'control-present', 'A' => 'control-absent', 'H' => 'control-late', 'T' => 'control-tardy');
    if ($current_RET[$THIS_RET['COLLEGE_ROLL_NO']][1]['ATTENDANCE_TEACHER_CODE'] == substr($title, 5)) {

        if ($_SESSION['PROFILE'] == 'admin')
            return "<TABLE align=center><TR><TD " . ($colors[$value] ? ' style=background-color:' . $colors[$value] : '') . "><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]] value='$title' CHECKED><span></span></label></TD></TR></TABLE>";
        else {
            if ($flag == true)
                return "<TABLE align=center><TR><TD " . ($colors[$value] ? ' style=background-color:' . $colors[$value] : '') . "><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]] value='$title' disabled=disabled CHECKED><span></span></label></TD></TR></TABLE>";
            else {
                $attn_code_type = DBGet(DBQuery('SELECT TYPE FROM attendance_codes WHERE ID=' . $current_RET[$THIS_RET['COLLEGE_ROLL_NO']][1]['ATTENDANCE_TEACHER_CODE']));
                if ($attn_code_type[1]['TYPE'] == 'official')
                    return "<TABLE align=center><TR><TD " . ($colors[$value] ? ' style=background-color:' . $colors[$value] : '') . "><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]] value='$title' disabled=disabled CHECKED><span></span></label></TD></TR></TABLE>";
        ...
        if ($_SESSION['PROFILE'] == 'admin')
            return "<TABLE align=center><TR><TD><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]] value='$title'" . (AllowEdit() ? '' : ' ') . " /><span></span></label></TD></TR></TABLE>";
        else {

            if ($flag == true)
                return "<TABLE align=center><TR><TD><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]]  disabled=disabled value='$title'" . (AllowEdit() ? '' : ' ') . "><span></span></label></TD></TR></TABLE>";
            else {
...
function _makeRadioSelected($value, $title) {
    ...
    if ($current_RET[$THIS_RET['COLLEGE_ROLL_NO']][1]['ATTENDANCE_TEACHER_CODE'] != '') {
        ...
        if ($attn_code_type[1]['TYPE'] == 'official' && $_SESSION['PROFILE'] != 'admin')
            return "<TABLE align=center><TR><TD " . ($colors1[$value] ? ' style=background-color:' . $colors1[$value] : '') . "><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]] value='$title' disabled=disabled CHECKED><span></span></label></TD></TR></TABLE>";
        else
            return "<TABLE align=center><TR><TD " . ($colors1[$value] ? ' style=background-color:' . $colors1[$value] : '') . "><label class=\"radio-inline m-0\"><INPUT class=\"" . $colors[$value] . "\" type=radio name=attendance[$THIS_RET[COLLEGE_ROLL_NO]] value='$title' CHECKED><span></span></label></TD></TR></TABLE>";
    }
}
...
function _makeTipMessage($value, $title) {
    ...
    if ($StudentPicturesPath && ($file = @fopen($picture_path = $StudentPicturesPath . '/' . $THIS_RET['COLLEGE_ROLL_NO'] . '.JPG', 'r') || $file = @fopen($picture_path = $StudentPicturesPath . '/' . $THIS_RET['COLLEGE_ROLL_NO'] . '.JPG', 'r')))
