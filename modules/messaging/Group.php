...
if (!isset($_REQUEST['modfunc'])) {
    ...
    $custom_header = "<h6 class=\"panel-title\">Groups</h6><div class=\"heading-elements\"><a href='#' class=\"btn btn-default heading-btn\" onclick='load_link(\"Modules.php?modname=" . strip_tags(trim($_REQUEST[modname])) . "&modfunc=add_group\");'><i class=\"fa fa-plus\"></i> Add Group</a></div>";
...
if (isset($_REQUEST['modfunc']) && $_REQUEST['modfunc'] == 'groupmember') {
    ...
    echo '<h6 class="panel-title">Group Members</h6>';
    echo '</div>';
    ...
    echo '<div class="row"><label class="col-md-4 control-label text-right">Group Name </label>';
    ...
    echo '<div class="row"><label class="col-md-4 control-label text-right">Description </label>';
    echo '<div class="col-md-8">';
    ...
    echo '<hr class="m-0" />';
    echo '<div id="members">';
    $custom_header = '<h6 class="panel-title">' . count($member_list) . ' Member' . (((count($member_list)) > 1) ? 's' : '') . '</h6><div class="heading-elements"><a class="btn btn-primary heading-btn" href="Modules.php?modname=' . $_REQUEST[modname] . '&modfunc=exist_group&group_name=' . $grp . '&desc=' . $groupdesc . '&grp_id=' . $gid . '"><i class="fa fa-plus"></i> Add Member</a></div>';
...
if (isset($_REQUEST['modfunc']) && $_REQUEST['modfunc'] == 'exist_group') {
    ...
    echo '<div class="panel panel-default">';
    echo '<div class="panel-heading">';
    echo '<h6 class="panel-title">Add Group Member</h6>';
    echo '</div>';
    echo '<div class="panel-body">';
    //PopTable('header', '');
    echo '<div class="row">';
...
if (isset($_REQUEST['modfunc']) && $_REQUEST['modfunc'] == 'add_group_member') {
    ...
    $allcolleges = $_REQUEST['_search_all_colleges'];
    ...
    if (isset($_REQUEST['group_id'])) {
        ...
        foreach ($usernames as $id => $value) {
            ...
            if ($value['PROFILE_ID'] == 3) {
                $stu = "select * from login_authentication,students where login_authentication.user_id=students.college_roll_no and profile_id=3 and USERNAME='$value[USERNAME]'";
        ...
        if ($profile != -1) {//search by profile  
            if ($profile == 3) {//students
                if (User('PROFILE') == 'teacher')
                    $user = "SELECT * FROM students,login_authentication WHERE profile_id=3 and login_authentication.user_id=students.college_roll_no and  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> ''  AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") AND college_roll_no IN (SELECT DISTINCT college_roll_no FROM course_periods INNER JOIN schedule USING ( course_period_id ) WHERE course_periods.teacher_id = " . UserID() . ")";
                elseif (User('PROFILE') == 'parent') {
                    $parent_id = UserID();
                    $qr = DBGet(DBQuery('Select COLLEGE_ROLL_NO from students_join_people where person_id=\'' . $parent_id . '\''));
                    $college_roll_no = $qr[1]['COLLEGE_ROLL_NO'];
                    $user = "SELECT * FROM students,login_authentication WHERE profile_id=3 and login_authentication.user_id=students.college_roll_no  and students.college_roll_no in (Select COLLEGE_ROLL_NO from students_join_people where person_id=" . $parent_id . ") and  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> ''  AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ")";
                } elseif (UserProfileID() == 1 || UserProfileID() == 5) {
                    $user = "select * from students,login_authentication,student_enrollment WHERE profile_id=3 and login_authentication.user_id=students.college_roll_no and students.college_roll_no=student_enrollment.college_roll_no and student_enrollment.college_id in(select college_id from staff_college_relationship where staff_id=" . UserID() . ") and  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ")";
                } else {

                    $user = "select * from students,login_authentication WHERE profile_id=3 and login_authentication.user_id=students.college_roll_no and  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ")";
                }
            }
            ...
            if ($profile == 2) {//teachers
                if (User('PROFILE') == 'parent') {
                    ...
                    $user = "SELECT * FROM login_authentication,staff WHERE login_authentication.user_id=staff.staff_id and login_authentication.profile_id=2 and staff.PROFILE_ID=$profile AND  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") AND staff_id  IN (SELECT distinct(course_periods.teacher_id) FROM course_periods,schedule where schedule.course_period_id=course_periods.course_period_id and schedule.college_roll_no in (Select COLLEGE_ROLL_NO from students_join_people where person_id=" . $parent_id . "))";
                } else if (User('PROFILE') == 'student') {
                    $studentId = UserStudentID();
                    $user = "SELECT * FROM login_authentication,staff WHERE login_authentication.user_id=staff.staff_id and login_authentication.profile_id=2 and staff.PROFILE_ID=$profile AND  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") AND staff_id IN(Select distinct teacher_id from course_periods INNER JOIN schedule using(course_period_id) where schedule.college_roll_no=" . $studentId . ")";
                } else if (UserProfileID() == 1 || UserProfileID() == 5) {
                    $user = "SELECT * FROM login_authentication,staff,staff_college_relationship WHERE login_authentication.user_id=staff.staff_id  and staff_college_relationship.staff_id=staff.staff_id  and staff_college_relationship.college_id in (select college_id from staff_college_relationship where staff_id=" . UserID() . ")  and login_authentication.profile_id=2 and staff.PROFILE_ID=$profile AND  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . " )";
                } else {
                    $user = "SELECT * FROM login_authentication,staff WHERE login_authentication.user_id=staff.staff_id and login_authentication.profile_id=2 and staff.PROFILE_ID=$profile AND  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . " )";
                }
            }
            if ($profile == 4) {//parents
                if (User('PROFILE') == 'teacher') {
                    ...
                    $user = 'SELECT * FROM login_authentication,people WHERE login_authentication.user_id=people.staff_id and login_authentication.profile_id=4 and people.profile_id=' . $profile . ' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=' . $groupselect[1]['GROUP_ID'] . ' ) and  TRIM( IFNULL( USERNAME, \'\' ) ) <> \'\' AND user_id IN (SELECT DISTINCT person_id FROM students_join_people WHERE college_roll_no=' . $college_roll_no . ' )';
                }
            }
            ...
            if (UserProfileID() == 1 || UserProfileID() == 5) {//all types of admin
                $user1 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,staff,staff_college_relationship WHERE login_authentication.user_id=staff.staff_id and staff_college_relationship.staff_id=staff.staff_id  and staff_college_relationship.college_id in (select college_id from staff_college_relationship where staff_id=" . UserID() . ") AND TRIM( IFNULL( USERNAME, '' ) ) <> '' AND TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") and login_authentication.profile_id not in(3,4)";
                $user2 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,students,student_enrollment WHERE login_authentication.user_id=students.college_roll_no and students.college_roll_no=student_enrollment.college_roll_no and student_enrollment.college_id in(select college_id from staff_college_relationship where staff_id=" . UserID() . ")AND login_authentication.profile_id=3 AND TRIM( IFNULL( USERNAME, '' ) ) <> '' AND TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") and login_authentication.profile_id=3";
                $user3 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,students_join_people,people  WHERE login_authentication.user_id=people.staff_id AND students_join_people.person_id in(select college_id from students,student_enrollment,students_join_people  where students.college_roll_no=student_enrollment.college_roll_no and students_join_people.college_roll_no=students.college_roll_no and student_enrollment.college_id in(select college_id from staff_college_relationship where  staff_id=" . UserID() . ")) and TRIM( IFNULL( USERNAME, '' ) ) <> '' AND TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") and login_authentication.profile_id=4";
            }
            if (User('PROFILE') == 'teacher') {//teachers
                $user1 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,staff,staff_college_relationship WHERE login_authentication.user_id=staff.staff_id and staff_college_relationship.staff_id=staff.staff_id AND TRIM( IFNULL( USERNAME, '' ) ) <> '' AND TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") and login_authentication.profile_id in(0,1,5) and college_id in(select college_id from staff_college_relationship where staff_id=" . UserID() . ")"; //all types of admin
                $user2 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM students,login_authentication WHERE profile_id=3 and login_authentication.user_id=students.college_roll_no and  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> ''  AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") AND college_roll_no IN (SELECT DISTINCT college_roll_no FROM course_periods INNER JOIN schedule USING ( course_period_id ) WHERE course_periods.teacher_id = " . UserID() . ")"; //scheduled students
                $user3 = 'SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,people WHERE login_authentication.user_id=people.staff_id and login_authentication.profile_id=4  AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=' . $groupselect[1]['GROUP_ID'] . ' ) and  TRIM( IFNULL( USERNAME, \'\' ) ) <> \'\' AND user_id IN (SELECT DISTINCT person_id FROM students_join_people WHERE college_roll_no IN (SELECT college_roll_no FROM students WHERE college_roll_no IN (SELECT DISTINCT college_roll_no FROM course_periods INNER JOIN schedule USING (course_period_id ) WHERE course_periods.teacher_id = \'' . UserID() . '\')))'; //parents                  
            }
            if (User('PROFILE') == 'parent') {//parents
                ...
                $user3 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM students,login_authentication WHERE profile_id=3 and login_authentication.user_id=students.college_roll_no  and students.college_roll_no in (Select COLLEGE_ROLL_NO from students_join_people where person_id=" . $parent_id . ") and  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> ''  AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ")";
            }
            if (User('PROFILE') == 'student') {//students
                $user1 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,staff WHERE login_authentication.user_id=staff.staff_id AND TRIM( IFNULL( USERNAME, '' ) ) <> '' AND TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") and login_authentication.profile_id in(0,1,5)"; //all types of admin

                $user2 = "SELECT DISTINCT username,LAST_NAME,first_name,login_authentication.profile_id,is_disable,login_authentication.user_id FROM login_authentication,staff WHERE login_authentication.user_id=staff.staff_id and login_authentication.profile_id=2  AND  TRIM( IFNULL( USERNAME, '' ) ) <> '' AND  TRIM( IFNULL( PASSWORD, '' ) ) <> '' AND USERNAME NOT IN(SELECT USER_NAME from mail_groupmembers where group_id=" . $groupselect[1]['GROUP_ID'] . ") AND staff_id IN(Select distinct teacher_id from course_periods INNER JOIN schedule using(course_period_id) where schedule.college_roll_no=" . UserStudentID() . ")"; //teachers                 
        ...
        if ($_REQUEST['_search_all_colleges'] != 'Y') {
            ...
            foreach ($userlist as $key => $value) {
                ...
                if ($userlist[$key]['PROFILE_ID'] == 3) {
                    $select = "SELECT  se.*,up.* FROM student_enrollment se,user_profiles up WHERE up.ID=" . $userlist[$key]['PROFILE_ID'] . " and se.college_id=" . UserCollege() . " AND se.college_roll_no='" . $userlist[$key]['USER_ID'] . "' group by college_roll_no";
                    $profile = DBGet(DBQuery($select));
                    ...
                    if (User('PROFILE') == 'student')
                        $select = "SELECT se.*,up.* FROM student_enrollment se,user_profiles up WHERE up.ID=" . $userlist[$key]['PROFILE_ID'] . " and se.college_id=" . UserCollege() . " AND se.college_roll_no=" . UserStudentID() . "";
                    if (User('PROFILE') == 'teacher')
                        $select = "SELECT se.*,up.* FROM student_enrollment se,user_profiles up WHERE up.ID=" . $userlist[$key]['PROFILE_ID'] . " and se.college_id=" . UserCollege() . " AND se.college_roll_no in (select schedule.college_roll_no from  schedule,course_periods,students_join_people where course_periods.course_period_id=schedule.course_period_id  and  schedule.college_roll_no=students_join_people.college_roll_no and students_join_people.person_id=" . $userlist[$key]['USER_ID'] . " and teacher_id=" . UserID() . ")";
                    else
                        $select = "SELECT se.*,up.* FROM student_enrollment se,user_profiles up WHERE up.ID=" . $userlist[$key]['PROFILE_ID'] . " and se.college_id=" . UserCollege() . " AND se.college_roll_no in (select college_roll_no from  students_join_people where person_id=" . $userlist[$key]['USER_ID'] . ") ";
                    $profile = DBGet(DBQuery($select));
                    ...
                    $select = "SELECT se.*,up.* FROM staff_college_relationship se,user_profiles up WHERE up.ID=" . $userlist[$key]['PROFILE_ID'] . " and se.college_id=" . UserCollege() . " AND se.staff_id='" . $userlist[$key]['USER_ID'] . "' group  by staff_id";
                    $profile = DBGet(DBQuery($select));
                    foreach ($profile as $k => $v) {
                        if (!in_array($userlist[$key]['USERNAME'], $exist_username_arr)) {
        ...
        if ($_REQUEST['_dis_user'] == 'Y') {
            foreach ($userlist as $ui => $ud) {
                ...
                elseif ($ud['PROFILE_ID'] == 'parent')
                    ...
                    $chck_status = DBGet(DBQuery('SELECT COUNT(1) as DISABLED FROM staff s,staff_college_relationship se WHERE se.STAFF_ID=s.STAFF_ID AND s.STAFF_ID=' . $ud['USER_ID'] . ' AND se.SYEAR=' . UserSyear() . ' AND (s.IS_DISABLE=\'Y\' OR (se.END_DATE<\'' . date('Y-m-d') . '\'  AND se.END_DATE IS NOT NULL AND se.END_DATE<>\'0000-00-00\' ))'));
