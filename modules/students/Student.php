...
if ($_REQUEST['month_values']['student_enrollment'][$_REQUEST['student_id']]['START_DATE'] != '' && $_REQUEST['day_values']['student_enrollment'][$_REQUEST['student_id']]['START_DATE'] != '' && $_REQUEST['year_values']['student_enrollment'][$_REQUEST['student_id']]['START_DATE'] != '') {
    ...
    if ($_REQUEST['values']['student_enrollment'][$_REQUEST['student_id']]['CALENDAR_ID'] != '') {
        ...
        $cal_id = DBGet(DBQuery('SELECT CALENDAR_ID FROM student_enrollment WHERE STUDENT_ID=' . $_REQUEST['student_id'] . ' AND SYEAR=' . UserSyear() . ' AND SCHOOL_ID=' . UserCollege() . ' ORDER BY ID DESC LIMIT 0,1'));
        $cal_id = $cal_id[1]['CALENDAR_ID'];
    }


    $get_c_dates = DBGet(DBQuery('SELECT MIN(SCHOOL_DATE) as START_DATE,MAX(SCHOOL_DATE) as END_DATE FROM attendance_calendar WHERE CALENDAR_ID=' . $cal_id . ' AND SCHOOL_ID=' . UserCollege() . ' AND SYEAR=' . UserSyear()));
...
//        else
//        {
        $get_sch = DBGet(DBQuery('SELECT * FROM schedule WHERE STUDENT_ID=' . $_REQUEST['student_id'] . ' AND SYEAR=' . UserSyear() . ' AND SCHOOL_ID=' . UserCollege() . ' AND (END_DATE IS NULL OR END_DATE>=\'' . date('Y-m-d') . '\') ORDER BY START_DATE ASC '));
        if (count($get_sch) > 0) {
...
/////////////////////////////////////////////////////////////////////////////////
        ...
        if ($_REQUEST['category_id'] == 3 && !isset($_REQUEST['address_id'])) {
            ...
            $address_id = DBGet(DBQuery("SELECT ID as ADDRESS_ID FROM student_address WHERE STUDENT_ID='" . UserStudentID() . "' AND SYEAR='" . UserSyear() . "' AND SCHOOL_ID='" . UserCollege() . "' AND TYPE='Home Address' "));
        ...
        AND SCHOOL_ID='" . UserCollege() . "' AND SYEAR='" . UserSyear() . "' AND ID=(SELECT MAX(ID) FROM student_enrollment WHERE STUDENT_ID='" . UserStudentID() . "')) AND se.SYEAR='" . UserSyear() . "' AND s.STUDENT_ID=se.STUDENT_ID";
...
//                                    }
                                    ...
                                    $stu_img_info = DBGet(DBQuery('SELECT * FROM user_file_upload WHERE USER_ID=' . $_REQUEST[student_id] . ' AND PROFILE_ID=3 AND SCHOOL_ID=' . UserCollege() . ' AND SYEAR=' . UserSyear() . ' AND FILE_INFO=\'stuimg\''));
...
//          $filename =  $upload->target_path;
                                        ...
                                        DBQuery('INSERT INTO user_file_upload (USER_ID,PROFILE_ID,SCHOOL_ID,SYEAR,NAME, SIZE, TYPE, CONTENT,FILE_INFO) VALUES (' . $_REQUEST[student_id] . ',\'3\',' . UserCollege() . ',' . UserSyear() . ',\'' . $fileName . '\', \'' . $fileSize . '\', \'' . $fileType . '\', \'' . $content . '\',\'stuimg\')');
                                    }
                                }
                        ...
                        if (count($_REQUEST['medical_info'])) {
                            $get_medical_info = DBGet(DBQuery('SELECT * FROM medical_info WHERE STUDENT_ID=' . $_REQUEST[student_id] . ' AND SYEAR=' . UserSyear() . ' AND SCHOOL_ID=' . UserCollege()));
                            if (count($get_medical_info) > 0) {
                                ...
                                $sql = substr($sql, 0, -1) . " WHERE STUDENT_ID='$_REQUEST[student_id]' AND SYEAR=" . UserSyear() . " AND SCHOOL_ID=" . UserCollege() . "";
                            }
                            else {
                        ...
                        $stu_enroll_id = DBGet(DBQuery('SELECT MAX(ID) AS M_ID FROM student_enrollment WHERE STUDENT_ID=\'' . $_REQUEST['student_id'] . '\' AND SYEAR=\'' . UserSyear() . '\' AND SCHOOL_ID=\'' . UserCollege() . '\''));
                        ...
                        if (count($_REQUEST['values']['student_enrollment'][$e_id])) {
                            $sql = 'SELECT ID,COURSE_ID,COURSE_PERIOD_ID,MARKING_PERIOD_ID FROM schedule WHERE STUDENT_ID=\'' . UserStudentID() . '\' AND SYEAR=\'' . UserSyear() . '\' AND SCHOOL_ID=\'' . UserCollege() . '\'';
                            ...
                            foreach ($_REQUEST['values']['student_enrollment'][$e_id] as $column_name => $value) {
                                ...
                                if ($column_name == 'START_DATE') {
                                    ...
                                    $last_college = $end_date[1]['LAST_SCHOOL'];
                                    $end_date = $end_date[1]['END_DATE'];
                                    if ($last_college != '') {
                                        $get_ls_drop_date = DBGet(DBQuery('SELECT END_DATE FROM student_enrollment WHERE STUDENT_ID=' . UserStudentID() . ' AND SCHOOL_ID=' . $last_college . ' ORDER BY ID DESC LIMIT 0,1'));
                            ...
                            $sql = substr($sql, 0, -1) . " WHERE STUDENT_ID='$_REQUEST[student_id]' AND SYEAR='" . UserSyear() . "' AND SCHOOL_ID='" . UserCollege() . "' AND iD='" . $e_id . "'";
//                            print_r($_REQUEST);
...
//          $filename =  $upload->target_path;
                                        ...
                                        DBQuery('INSERT INTO user_file_upload (USER_ID,PROFILE_ID,SCHOOL_ID,SYEAR,NAME, SIZE, TYPE, CONTENT,FILE_INFO) VALUES (' . $last_student_id . ',\'3\',' . UserCollege() . ',' . UserSyear() . ',\'' . $fileName . '\', \'' . $fileSize . '\', \'' . $fileType . '\', \'' . $content . '\',\'stuimg\')');
                                ...
                                DBQuery('INSERT INTO medical_info (STUDENT_ID,SYEAR,SCHOOL_ID) VALUES (' . $max_stId[1]['STU_ID'] . ',' . UserSyear() . ',' . UserCollege() . ')');
        ...
        if ($flag != true) {
            ...
            if (User('PROFILE') == 'admin') {
                ...
                if ($_REQUEST['_search_all_colleges'] == 'Y' || $_SESSION['_search_all'] == 1) {
                    ...
                    $admin_COMMON_WHERE = " WHERE s.STUDENT_ID=ssm.STUDENT_ID  AND a.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=" . UserSyear() . " AND ssm.SCHOOL_ID IN (" . GetUserColleges(UserID(), true) . ") ";
                    $_SESSION['_search_all'] = 1;
                } else {
                    $admin_COMMON_WHERE = " WHERE s.STUDENT_ID=ssm.STUDENT_ID  AND a.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=" . UserSyear() . " AND ssm.SCHOOL_ID=" . UserCollege() . " ";
                }
	...
	schedule ss,student_address a ";
                $teacher_COMMON_WHERE = " WHERE a.STUDENT_ID=s.STUDENT_ID AND a.TYPE='Home Address' AND s.STUDENT_ID=ssm.STUDENT_ID AND ssm.STUDENT_ID=ss.STUDENT_ID AND ssm.SYEAR=cp.SYEAR AND ssm.SYEAR=ss.SYEAR AND cp.COURSE_ID=ss.COURSE_ID AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.MARKING_PERIOD_ID IN (" . GetAllMP('', $queryMP) . ")
						AND (cp.TEACHER_ID='" . User('STAFF_ID') . "' OR cp.SECONDARY_TEACHER_ID='" . User('STAFF_ID') . "') AND cp.COURSE_PERIOD_ID='" . UserCoursePeriod() . "' AND ssm.SYEAR=" . UserSyear() . " AND ssm.SCHOOL_ID=" . UserCollege() . " ";

                if ($_REQUEST['_search_all_colleges'] == 'Y' || $_SESSION['_search_all'] == 1) {
                    $teacher_COMMON_WHERE = " WHERE a.STUDENT_ID=s.STUDENT_ID AND a.TYPE='Home Address'  AND s.STUDENT_ID=ssm.STUDENT_ID AND ssm.STUDENT_ID=ss.STUDENT_ID AND ssm.SYEAR=cp.SYEAR AND ssm.SYEAR=ss.SYEAR AND cp.COURSE_ID=ss.COURSE_ID AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.MARKING_PERIOD_ID IN (" . GetAllMP('', $queryMP) . ")
						AND (cp.TEACHER_ID='" . User('STAFF_ID') . "' OR cp.SECONDARY_TEACHER_ID='" . User('STAFF_ID') . "') AND cp.COURSE_PERIOD_ID='" . UserCoursePeriod() . "' AND ssm.SYEAR=" . UserSyear() . " AND ssm.SCHOOL_ID IN (" . GetUserColleges(UserID(), true) . ") ";
                    $_SESSION['_search_all'] = 1;
                } else {
                    $teacher_COMMON_WHERE = " WHERE a.STUDENT_ID=s.STUDENT_ID AND a.TYPE='Home Address' AND s.STUDENT_ID=ssm.STUDENT_ID AND ssm.STUDENT_ID=ss.STUDENT_ID AND ssm.SYEAR=cp.SYEAR AND ssm.SYEAR=ss.SYEAR AND cp.COURSE_ID=ss.COURSE_ID AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.MARKING_PERIOD_ID IN (" . GetAllMP('', $queryMP) . ")
						AND (cp.TEACHER_ID='" . User('STAFF_ID') . "' OR cp.SECONDARY_TEACHER_ID='" . User('STAFF_ID') . "') AND cp.COURSE_PERIOD_ID='" . UserCoursePeriod() . "' AND ssm.SYEAR=" . UserSyear() . " AND ssm.SCHOOL_ID=" . UserCollege() . " ";
            ...
            if (UserStudentID() || $_REQUEST['student_id'] == 'new') {
                ...
                if (clean_param($_REQUEST['modfunc'], PARAM_NOTAGS) != 'delete' || $_REQUEST['delete_ok'] == '1') {
                    ...
                    WHERE s.STUDENT_ID='" . UserStudentID() . "' AND s.STUDENT_ID=la.USER_ID AND la.PROFILE_ID=3";
                        ...
                        $college = DBGet(DBQuery("SELECT SCHOOL_ID,GRADE_ID FROM student_enrollment WHERE STUDENT_ID='" . UserStudentID() . "' AND SYEAR='" . UserSyear() . "' AND ('" . DBDate() . "' BETWEEN START_DATE AND END_DATE OR END_DATE IS NULL)"));
