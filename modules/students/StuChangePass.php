...
{
	$stu_PASS=DBGet(DBQuery('SELECT PASSWORD FROM students WHERE COLLEGE_ROLL_NO=\''.UserStudentId().'\''));
	  ...
	  if($stu_PASS[1]['PASSWORD']==$pass_old)
	   {
		 ...
		 {
	 	  $sql='UPDATE students SET PASSWORD=\''.$pass_new.'\' WHERE COLLEGE_ROLL_NO=\''.UserStudentId().'\'';
		  DBQuery($sql);
