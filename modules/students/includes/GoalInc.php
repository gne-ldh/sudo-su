...
$sql_urg_sch_id = 'SELECT college_id FROM student_goal WHERE goal_id = ' . clean_param($_REQUEST['goal_id'], PARAM_INT);
...
if ($_REQUEST['modfunc'] != 'delete' && !$_REQUEST['goal_id']) {
    $subjects_RET = DBGet(DBQuery('SELECT GOAL_ID,GOAL_TITLE FROM student_goal WHERE COLLEGE_ID=\'' . UserCollege() . '\' AND SYEAR=\'' . UserSyear() . '\' AND COLLEGE_ROLL_NO=\'' . UserStudentID() . '\''));
    ...
    foreach ($_REQUEST['tables'] as $table_name => $tables) {
        foreach ($tables as $id => $columns) {
            if ($id != 'new' && $i == 0) {
                ...
                $college_syear_date = DBGet(DBQuery('SELECT * from college_years where syear=\'' . UserSyear() . '\' and college_id=\'' . UserCollege() . '\''));

                $college_start_dt = $college_syear_date[1]['START_DATE'];
                $college_end_dt = $college_syear_date[1]['END_DATE'];
               
                if ($table_name!='student_goal_progress' && ((strtotime($start_date) > strtotime($end_date) && $start_date != "" && $end_date != "") || (strtotime($row_s_date[1]['START_DATE']) > strtotime($end_date) && $row_s_date[1]['START_DATE'] != "" && $end_date != "") || (strtotime($start_date) > strtotime($row_s_date[1]['END_DATE']) && $start_date != "" && $row_s_date[1]['END_DATE'] != "") || (strtotime($row_s_date[1]['START_DATE']) > strtotime($row_s_date[1]['END_DATE']) && $row_s_date[1]['START_DATE'] != "" && $row_s_date[1]['END_DATE'] != "") || (strtotime($start_date) < strtotime($college_start_dt) || strtotime($start_date) > strtotime($college_end_dt)) || (strtotime($end_date) < strtotime($college_start_dt) || strtotime($end_date) > strtotime($college_end_dt)))) {
                {
                ...
                if ($table_name == 'student_goal') {
                    ...
                    $fields = 'COLLEGE_ROLL_NO,COLLEGE_ID,SYEAR,START_DATE,END_DATE,';
                    ...
                    $id[1]['ID'] = $id[1]['AUTO_INCREMENT'];
                    $fields = 'GOAL_ID,COLLEGE_ROLL_NO,START_DATE,';
                ...
                if ($go) {
                    if ($table_name == 'student_goal_progress') {
                        ...
                        $college_start_dt = $college_syear_date[1]['START_DATE'];
                        $college_end_dt = $college_syear_date[1]['END_DATE'];
                        if ((isset($start_date) && isset($end_date) && strtotime($end_date) < strtotime($start_date)) || (strtotime($start_date) < strtotime($college_start_dt) || strtotime($start_date) > strtotime($college_end_dt)) || (strtotime($end_date) < strtotime($college_start_dt) || strtotime($end_date) > strtotime($college_end_dt))) {
                            ShowErr('Data not saved because start and end date is not valid');
                        } else {
...
if ((!clean_param($_REQUEST['modfunc'], PARAM_NOTAGS) || clean_param($_REQUEST['modfunc'], PARAM_NOTAGS) == 'choose_course') && !clean_param($_REQUEST['course_modfunc'], PARAM_NOTAGS)) {
    ...
    $sql = 'SELECT GOAL_ID,GOAL_TITLE FROM student_goal WHERE COLLEGE_ID=\'' . $college_id . '\' AND SYEAR=\'' . UserSyear() . '\' AND COLLEGE_ROLL_NO=\'' . UserStudentID() . '\' ORDER BY START_DATE DESC';
    ...
    $sql_cp = 'SELECT cp.COURSE_PERIOD_ID AS COURSE_PERIOD, cp.TITLE AS COURSE_PERIOD_NAME FROM course_periods cp, schedule s WHERE s.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND s.COLLEGE_ROLL_NO=\'' . UserStudentID() . '\'';
        ...
        if ($_REQUEST['progress_id']) {
            ...
            if (($_REQUEST['progress_id'] != 'new')) {
                if (((User('PROFILE') == 'admin') && isset($edit_per_adm[1]['CAN_EDIT'])) || ((User('PROFILE') == 'teacher') && isset($edit_per_teach[1]['CAN_EDIT'])) || ((User('PROFILE') == 'parent') && isset($edit_per_prnt[1]['CAN_EDIT']))) {
                    $header .= "<div class=\"col-md-6 text-right\"><a class=\"btn btn-danger btn-labeled\" href='Modules.php?modname=students/Student.php&include=GoalInc&category_id=5&action=delete&gid=" . $row_gid[1]["GOAL_ID"] . "&pid=" . $_REQUEST[progress_id] . "'><b><i class=\"icon-trash\"></i></b> Delete This Progress</a></div>"; // DateInput is copied from collegesetup/MarkingPeriods.php line 295
                }
            }
            $header .= '</div>'; //.row
            
            $header .= '<div class="row">';
            $header .= '<div class="col-md-6"><div class="form-group">' . SelectInput($RET['GOAL_ID'] ? $RET['GOAL_ID'] : $_REQUEST['goal_id'], 'tables[student_goal_progress][' . $_REQUEST['progress_id'] . '][GOAL_ID]', 'Goal Title', $options, false).'</div></div>'; // DateInput is copied from collegesetup/MarkingPeriods.php line 295
            $header .= '<div class="col-md-6"><div class="form-group">' . SelectInput($RET_sel['COURSE_PERIOD_ID'], 'tables[student_goal_progress][' . $_REQUEST['progress_id'] . '][COURSE_PERIOD_ID]', 'Course Period', $options_cp) . '</div></div>'; // DateInput is copied from collegesetup/MarkingPeriods.php line 295
            $header .= '</div>'; //.row
            ...
            $header .= '<div class="row">';
            $header .= '<div class="col-md-6"><div class="form-group"><label class="control-label text-right col-lg-4">Date of Entry</label><div class="col-lg-8">' . DateInputAY($RET['START_DATE']!="" ? $RET['START_DATE'] : "", 'tables[' . $_REQUEST['progress_id'] . '][START_DATE]', 1) . '</div></div></div>'; // DateInput is copied from collegesetup/MarkingPeriods.php line 295
            ...
            if ($_REQUEST['goal_id'] != 'new') {
                if (((User('PROFILE') == 'admin') && isset($edit_per_adm[1]['CAN_EDIT'])) || ((User('PROFILE') == 'teacher') && isset($edit_per_teach[1]['CAN_EDIT'])) || ((User('PROFILE') == 'parent') && isset($edit_per_prnt[1]['CAN_EDIT']))) {
                    $header .= "<div class=\"col-md-6 text-right\"><a class=\"btn btn-danger btn-labeled\" href='Modules.php?modname=students/Student.php&include=GoalInc&category_id=5&action=delete_goal&gid=" . $_REQUEST['goal_id'] . "'><b><i class=\"icon-cross\"></i></b> Delete This Goal</a></div>"; // DateInput is copied from collegesetup/MarkingPeriods.php line 295
                }
            }
            $header .= '</div>'; //.row
            ...
            if ($_REQUEST[goal_id] . trim() != '')
                $header .= '<div class="col-md-6"><div class="form-group"><label class="control-label col-lg-4 text-right">Begin Date</label><div class="col-lg-8"><input type="hidden" name="goalId" id="goalId" value="' . $_REQUEST[goal_id] . '" />' . DateInputAY($RET['START_DATE']!="" ? $RET['START_DATE'] : "", 'tables[' . $_REQUEST['goal_id'] . '][START_DATE]', 2) . '</div></div></div>'; // DateInput is copied from collegesetup/MarkingPeriods.php line 295
            else
                $header .= '<div class="col-md-6"><div class="form-group"><label class="control-label col-lg-4 text-right">Begin Date</label><div class="col-lg-8"><input type="hidden" name="goalId" id="goalId" value="new" />' . DateInputAY($RET['START_DATE']!="" ? $RET['START_DATE'] : "", 'tables[' . $_REQUEST['goal_id'] . '][START_DATE]', 2) . '</div></div></div>'; // DateInput is copied from collegesetup/MarkingPeriods.php line 295

            $header .= '<div class="col-md-6"><div class="form-group"><label class="control-label col-lg-4 text-right">End Date</label><div class="col-lg-8">' . DateInputAY($RET['END_DATE']!="" ? $RET['END_DATE'] : "", 'tables[' . $_REQUEST['goal_id'] . '][END_DATE]', 3) . '</div></div></div>'; // DateInput is copied from collegesetup/MarkingPeriods.php line 296
            $header .= '</div>'; //.row
    ...
    if ($_REQUEST['goal_id'] && $_REQUEST['goal_id'] != 'new') {
        ...
        $sql = "SELECT PROGRESS_ID,PROGRESS_NAME FROM student_goal_progress WHERE GOAL_ID='" . $sql_goal_fetch[1]['GOAL_ID'] . "' AND COLLEGE_ROLL_NO=" . UserStudentID() . " ORDER BY START_DATE DESC";
        $QI = DBQuery($sql);
